 ...
 * OTP parameter block
 * 
 * The OTP parameter block save the configuration data of each slot. The size 
 * of each slot is 64 byte
 * 
 * Slot Start End GLOBAL_CONFIG 0 2 free 3 63 HOTP_SLOT1 64 127 HOTP_SLOT2
 * 128 191 TOTP_SLOT1 192 255 TOTP_SLOT2 256 319 TOTP_SLOT3 320 383
 * TOTP_SLOT4 384 447 TOTP_SLOT5 448 511 TOTP_SLOT6 512 575 TOTP_SLOT7 576
 * 639 TOTP_SLOT8 640 703 TOTP_SLOT9 704 767 TOTP_SLOT10 768 831 TOTP_SLOT11
 * 832 895 TOTP_SLOT12 896 959 TOTP_SLOT13 960 1023 TOTP_SLOT14 1024 1087
 * TOTP_SLOT15 1088 1151 TOTP_SLOT16 1152 1215
 * 
 * 
 ...
 * Slot size 64 byte
 * 
 * Contain the parameter data - 50 data byte Start Description
 * SLOT_TYPE_OFFSET 0 1 byte slot type TOTP, HOTP SLOT_NAME_OFFSET 1 15 byte
 * slot name SECRET_OFFSET 16 20 byte secret key CONFIG_OFFSET 36 1 byte
 * config byte TOKEN_ID_OFFSET 37 12 byte token ID
 * 
 * Stick 2.x
 * 
 * OTP parameter block
 * 
 * The OTP parameter block save the configuration data of each slot. The size 
 * of each slot is 80 byte
 * 
 * Slot Start End GLOBAL_CONFIG 0 2 free 3 63
 * 
 * HOTP_SLOT1 64 127 HOTP_SLOT2 128 191 HOTP_SLOT3 192 255
 * 
 * TOTP_SLOT1 256 335 TOTP_SLOT2 336 415 TOTP_SLOT3 416 495 TOTP_SLOT4 496
 * 575 TOTP_SLOT5 576 655 TOTP_SLOT6 656 735 TOTP_SLOT7 736 815 TOTP_SLOT8
 * 816 895 TOTP_SLOT9 896 975 TOTP_SLOT10 976 1055 TOTP_SLOT11 1056 1135
 * TOTP_SLOT12 1136 1215 TOTP_SLOT13 1216 1295 TOTP_SLOT14 1296 1375
 ...
 * SLOT_TYPE_OFFSET 0 1 byte slot type TOTP, HOTP SLOT_NAME_OFFSET 1 15 byte
 * slot name SECRET_OFFSET 16 20 byte secret key CONFIG_OFFSET 36 1 byte
 * config byte TOKEN_ID_OFFSET 37 12 byte token ID INTERVAL_OFFSET 50 8 byte
 * (Only HOPT)
 * 
 * 
 * OTP counter storage slot
 * 
 * This field is used for storing the actual counter value. Because of the
 * limitation of flash erase accesses (1000 for a Stick 1.4 flash page (1024
 * byte), 100000 for a Stick 2.0 flash page (512 byte)). it is necessary to
 * reduce the erase access of the flash. This is done by using a greater area 
 * of the flash. The limitation of flash accesses is only in the change of
 * the bits to 1 in a flash page. Only all bit in a hole flash page can set
 * to 1 in a single process (this is the erase access). The setting to 0 of a 
 * bit in the flash page is independent to other bits.
 * 
 * The implementation: The first 8 byte of the slot contains the base counter 
 * of stored value as an unsigned 64 bit value. The remaining page stored a
 * token per flash byte for a used number. When all tokens in a slot are
 * used, the base counter is raised and the tokens are reseted to 0xff
 * 
 * Flash page layout
 * 
 * Entry Position Counter base 0 - 7 8 byte, unsigned 64 bit Use flags 8 -
 * 1023 1016 byte marked a used value (for Stick 1.4) Use flags 8 - 511 504
 * byte marked a used value (for Stick 2.0)
 * 
 * Flash page byte order 0 1 2 3 4 End of flash page
 * 01234567890123456789012345678901234567890123456789.... X
 * VVVVVVVVFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF.... .
 * 
 * V = 64 bit counter base value F = token for a used value, 0xFF = unused,
 * 0x00 = used
 * 
 * The actual counter is the sum of the counter base value and the number of
 * tokens with a 0 in the slot.
 * 
 * Example:
 * 
 * Flash page byte order 0 1 2 3 4 End of flash page
 * 01234567890123456789012345678901234567890123456789.... X xHi
 * 0000000000000000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF.... . xLo
 * 0000001000000000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF.... .
 * 
...
/*
 * u32 hotp_slots[NUMBER_OF_HOTP_SLOTS] = {SLOTS_ADDRESS + HOTP_SLOT1_OFFSET, 
 * SLOTS_ADDRESS + HOTP_SLOT2_OFFSET}; u32
 * hotp_slot_counters[NUMBER_OF_HOTP_SLOTS] = {SLOT1_COUNTER_ADDRESS,
 * SLOT2_COUNTER_ADDRESS}; u32 hotp_slot_offsets[NUMBER_OF_HOTP_SLOTS] =
 * {HOTP_SLOT1_OFFSET, HOTP_SLOT2_OFFSET};
 * 
 * u32 totp_slots[NUMBER_OF_TOTP_SLOTS] = {SLOTS_ADDRESS + TOTP_SLOT1_OFFSET, 
 * SLOTS_ADDRESS + TOTP_SLOT2_OFFSET, SLOTS_ADDRESS + TOTP_SLOT3_OFFSET,
 * SLOTS_ADDRESS + TOTP_SLOT4_OFFSET}; u32
 * totp_slot_offsets[NUMBER_OF_TOTP_SLOTS] = {TOTP_SLOT1_OFFSET,
 * TOTP_SLOT2_OFFSET, TOTP_SLOT3_OFFSET, TOTP_SLOT4_OFFSET}; 
 */
...
u32 *p;
     ...
     * { u8 text[30];
     * 
     * CI_StringOut ("get_time_value: time "); itoa (time,text); CI_StringOut 
     * ((char*)text);
     * 
     * CI_StringOut (" = "); itoa (crc_STM32 (time>>8),text); CI_StringOut
     * ((char*)text); CI_StringOut ("\r\n"); } 
     */
    return (time);
}
...
u32 time_1 = time;
    ...
    for (i = 0; i < 32; i++)
    {
        ...
        {
            flashc_memcpy ((void *) (TIME_ADDRESS + TIME_OFFSET * i),
                           (void *) &time, 4, TRUE);
            flag = 1;
            break;
        }
...
s32 page_s32;
    ...
    flashc_memcpy ((void *) addr, (void *) &counter, 8, TRUE);  // set the
                                                                // counter
                                                                // base value
...
FLASH_Status err = FLASH_COMPLETE;
    ...
    if (ptr[FLASH_PAGE_SIZE - 1] == 0x00)   // Are all token used ?
    {
        ...
        flashc_memset8 ((void *) BACKUP_PAGE_ADDRESS, 0xFF, FLASH_PAGE_SIZE,
                        TRUE);

        // write address to backup page
        flashc_memcpy ((void *) BACKUP_PAGE_ADDRESS, (void *) &counter, 8, FALSE);  // Area 
                                                                                    // is 
                                                                                    // erased

        // Write page addr to backup page
        flashc_memcpy ((void *) (BACKUP_PAGE_ADDRESS + BACKUP_ADDRESS_OFFSET), (void *) &addr, 4, FALSE);   // Area 
                                                                                                            // is 
                                                                                                            // erased


        dummy_u32 = 8;
        flashc_memcpy ((void *) (BACKUP_PAGE_ADDRESS + BACKUP_LENGTH_OFFSET), (void *) &dummy_u32, 4, FALSE);   // Area 
                                                                                                                // is 
                                                                                                                // erased

        // New erase by flash memset
        flashc_memset8 ((void *) addr, 0xFF, FLASH_PAGE_SIZE, TRUE);    // Erase 
                                                                        // counter 
                                                                        // page

        // Write counter page value
        flashc_memcpy ((void *) addr, (void *) &counter, 8, FALSE); // Area
                                                                    // is
                                                                    // srased

        // Write valid token to backup page
        dummy_u16 = 0x4F4B;
        flashc_memcpy ((void *) (BACKUP_PAGE_ADDRESS + BACKUP_OK_OFFSET), (void *) &dummy_u16, 2, FALSE);   // Area 
                                                                                                            // is 
                                                                                                            // erased
...
u8 text[20];
         ...
         * { u8 text[20];
         * 
         * CI_LocalPrintf ("After change0:" ); itoa ((u32)n,text);
         * CI_LocalPrintf (text); CI_LocalPrintf ("\n\r"); } 
         */
...
u8 config = 0;
    ...
    /*
     * For a counter test without flash { static u32 co = 0; counter = co;
     * co++; } 
     */
#ifdef DEBUG_HOTP
    {
...
u32 c;
    ...
    result =
        get_hotp_value (counter, (u8 *) (hotp_slots[slot] + SECRET_OFFSET),
                        20, len);
...
u32 dummy_u32;
    ...
    dummy_u16 = len;
    flashc_memcpy ((void *) (BACKUP_PAGE_ADDRESS + BACKUP_LENGTH_OFFSET), (void *) &dummy_u16, 2, FALSE);   // Area 
                                                                                                            // is 
                                                                                                            // erased

    dummy_u32 = addr;
    flashc_memcpy ((void *) (BACKUP_PAGE_ADDRESS + BACKUP_ADDRESS_OFFSET), (void *) &dummy_u32, 4, FALSE);  // Area 
                                                                                                            // is 
                                                                                                            // erased
...
u8 *page = (u8 *) SLOTS_ADDRESS;
    ...
    // Init backup block
    dummy_u16 = 0x4F4B;
    flashc_memcpy ((void *) (BACKUP_PAGE_ADDRESS + BACKUP_OK_OFFSET),
                   (void *) &dummy_u16, 2, TRUE);
...
u16 length = getu16 ((u8 *) BACKUP_PAGE_ADDRESS + BACKUP_LENGTH_OFFSET);
    ...
    else
    {
        ...
        if ((address != 0xffffffff) && (length <= 1000))    // todo 1000 >
                                                            // define
        {
            // New erase by flash memset
            ...
            // New erase by flash memset
            flashc_memset8 ((void *) BACKUP_PAGE_ADDRESS, 0xFF,
...
time_t now;
    ...
    // result= get_hotp_value(challenge,(u8
    // *)(totp_slots[slot]+SECRET_OFFSET),20,len);
    result =
        get_hotp_value (time_min, (u8 *) (totp_slots[slot] + SECRET_OFFSET),
                        20, len);
