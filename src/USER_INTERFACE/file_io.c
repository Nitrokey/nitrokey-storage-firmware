...
/*
 * #define HTML_FILEIO_LUN 0 // RAM Disk #define SD_UNCRYPTED_FILEIO_LUN 1
...
#define FILEIO_IMAGE_SIZE        (498*512)  // 0x3E400 // Don't save the
                                            // hidden volume and OTP data
                                            // area below - 0x40000 - // 256
                                            // k
...
s32 Ret_s32;
    ...
    for (i = 0; i < (FILEIO_IMAGE_SIZE / FILEIO_IMAGE_BLOCKSIZE); i++)
    {
        ...
        Ret_s32 =
            write (FileID_s32,
                   (u8 *) (FILEIO_IMAGE_START + i * FILEIO_IMAGE_BLOCKSIZE),
                   FILEIO_IMAGE_BLOCKSIZE);
    }
...
*******************************************************************************/
/*
 * #define FILEIO_STATUS_DIR "status" #define FILEIO_IMAGE_FILE
 * "status\\app.bin" #define FILEIO_IMAGE_START 0x80000000 #define
 * FILEIO_IMAGE_SIZE (498*512) // 0x3E400 // Don't save the hidden volume and 
 * OTP data area below - 0x40000 - // 256 k #define FILEIO_IMAGE_BLOCKSIZE
 * 4096
 * 
 ...
 * 
 * // Open File FileID_s32 = open
 * ((char*)FILE;O_IMAGE_FILE,O_CREAT|O_APPEND); if (0 > FileID_s32) {
 * CI_LocalPrintf ("Error open %s - %d -
 * %s\r\n",FILEIO_IMAGE_FILE,fs_g_status,
 * IBN_FileSystemErrorText(fs_g_status)); return (FALSE); }
 * 
 * // Write data for (i=0;i<(FILEIO_IMAGE_SIZE/FILEIO_IMAGE_BLOCKSIZE);i++) {
 * CI_LocalPrintf ("Write block = %3d\r\n",i); Ret_s32 = write
 * (FileID_s32,(u8*)(FILEIO_IMAGE_START + i *
 * FILEIO_IMAGE_BLOCKSIZE),FILEIO_IMAGE_BLOCKSIZE); }
 * 
 * // Close file Ret_s32 = close (FileID_s32); if (-1 == Ret_s32) {
 * CI_LocalPrintf ("Close = %d \r\n",Ret_s32); }
 * 
...
u8 InitStatusFiles_u8 (void)
{
    ...
    // Write test
    FileIO_AppendText_u8 ((u8 *) FILEIO_STATUS_FILE,
                          (u8 *) FILEIO_STATUS_TEXT_START);
...
void IBN_FileIo_Tests (u8 nParamsGet_u8, u8 CMD_u8, u32 Param_u32,
