...
// onclick='saveFile('data.txt',this.form.kennwort.value)'>"
...
u8 *HTML_Code_Startpage3_pu8 = {
    "  </p>"
        ...
        "   var Drive = Path.substr(1,2);" "   var pName = fileName;"
    ...
    "     file.create( Components.interfaces.nsIFile.NORMAL_FILE_TYPE, 420 );"
        ...
        "  outputStream.close();" "}" "</script>" "</body>" "</html>"
};
...
{
    // For firefox
    HTML_FileIO_AppendText_u8 ((u8 *) HTML_FILENAME_STARTPAGE_FF,
                               HTML_Code_Startpage1_pu8);
    HTML_FileIO_AppendText_u8 ((u8 *) HTML_FILENAME_STARTPAGE_FF,
                               HTML_Code_Startpage2_pu8);
    HTML_FileIO_AppendText_u8 ((u8 *) HTML_FILENAME_STARTPAGE_FF,
                               HTML_Code_Startpage3_pu8);


    // For IE
    HTML_FileIO_AppendText_u8 ((u8 *) HTML_FILENAME_STARTPAGE_IE,
                               HTML_Code_Startpage1_pu8);
    HTML_FileIO_AppendText_u8 ((u8 *) HTML_FILENAME_STARTPAGE_IE,
                               HTML_Code_Startpage2_pu8);
    HTML_FileIO_AppendText_u8 ((u8 *) HTML_FILENAME_STARTPAGE_IE,
                               HTML_Code_Startpage3_IE_pu8);
...
s32 Ret_s32 = -1;
    ...
    {
        CI_TickLocalPrintf
            ("HTML_CheckInput: Error mounting ram disk - %d - %s\r\n",
             fs_g_status, IBN_FileSystemErrorText (fs_g_status));
        return (FALSE);
    }
    ...
    if (0 > FileID_s32)
    {
        // CI_TickLocalPrintf ("HTML_CheckInput: Error open %s - %d -
        // %s\r\n",HTML_FILENAME_INPUTFILE,fs_g_status,
    ...
    if (TRUE == HTML_CheckInput (Text_u8))
    {
        ...
        switch (HTML_Command_s32)
        {
            ...
            case HTML_CMD_ENABLE_AES_LUN:
                if (TRUE ==
                    IW_SendToSC_PW1 (&Text_u8[HTML_CMD_STARTBYTE_OF_PAYLOAD]))
                {
                    GetStorageKey_u32 (&Text_u8
                                       [HTML_CMD_STARTBYTE_OF_PAYLOAD],
                                       StorageKey_pu8);
            ...
            case HTML_CMD_ENABLE_FIRMWARE_UPDATE:
                if (TRUE ==
                    IW_SendToSC_PW3 (&Text_u8[HTML_CMD_STARTBYTE_OF_PAYLOAD]))
                {
                    CI_TickLocalPrintf
                        ("Firmware update enabled - good bye\r\n");
            ...
            case HTML_CMD_FILL_SD_WITH_RANDOM_CHARS:
                ...
                {
                    CI_TickLocalPrintf
                        ("Fill SD card with random numbers - ist lasts very long, 1 GB ca. 4 minutes\r\n");
            ...
            case HTML_CMD_GENERATE_NEW_KEYS:
                ...
                BuildStorageKeys_u32 ((u8 *) &
                                      Text_u8[HTML_CMD_STARTBYTE_OF_PAYLOAD]);
                break;
            ...
            case HTML_CMD_ENABLE_READONLY_UNCRYPTED_LUN:
                CI_TickLocalPrintf
                    ("Undo readwrite enable uncrypted lun\r\n");
            ...
            default:
                CI_TickLocalPrintf
                    ("HTML_CheckRamDisk: Get unknown command: %d \r\n",
                     HTML_Command_s32);
                break;
        }
    }
...
void HID_ExcuteCmd (void)
{
        ...
        case HTML_CMD_CLEAR_NEW_SD_CARD_FOUND:
            switch (HID_String_au8[0])
            {
                ...
                case 'M':  // matrix password
                    ...
                    if (FALSE ==
                        ConvertMatrixDataToPassword (&HID_String_au8[1]))
                    {
                        UpdateStick20Command
                            (OUTPUT_CMD_STICK20_STATUS_WRONG_PASSWORD, 0);
                        return;
                    }
                    break;
            }
        ...
        case HTML_CMD_FILL_SD_WITH_RANDOM_CHARS:
            switch (HID_String_au8[1])
            {
                ...
                case 'M':  // matrix password
                    // Convert matrix input to password
                    if (FALSE ==
                        ConvertMatrixDataToPassword (&HID_String_au8[2]))
                    {
                        UpdateStick20Command
                            (OUTPUT_CMD_STICK20_STATUS_WRONG_PASSWORD, 0);
                        return;
    ...
    if (TRUE == HID_NextPasswordIsHiddenPassword_u32)
    {
        switch (Cmd_u8)
        {
            case HTML_CMD_ENABLE_AES_LUN:
                ...
                CI_TickLocalPrintf
                    ("Redirect password -%s- to hidden volume\r\n",
                     &HID_String_au8[1]);
                SetSdEncryptedCardEnableState (FALSE);
                SetSdEncryptedHiddenState (FALSE);
                ...
                UpdateStick20Command
                    (OUTPUT_CMD_STICK20_STATUS_BUSY_PROGRESSBAR, 50);
    ...
    switch (Cmd_u8)
    {
        ...
        case HTML_CMD_ENABLE_AES_LUN:
            ...
            {
                SetSdEncryptedHiddenState (FALSE);
                SetSdEncryptedCardEnableState (FALSE);  // Disable crypted or 
                                                        // hidden volume
                vTaskDelay (3000);
                ...
                /*
                 * HID_NextPasswordIsHiddenPassword_u32 = TRUE; // switch on
                 * redirection of aes volume password CI_TickLocalPrintf
                 * ("Next AES volume password is used for hidden
                 * volume\r\n"); 
                 */
                CI_TickLocalPrintf ("SD card AES LUN enabled\r\n");
            }
        ...
        case HTML_CMD_ENABLE_FIRMWARE_UPDATE:
            ...
            {
                CI_TickLocalPrintf
                    ("Security bit is active. Update not enabled\r\n");
                UpdateStick20Command
                    (OUTPUT_CMD_STICK20_STATUS_SECURITY_BIT_ACTIVE, 0);
                break;
            }
        ...
        case HTML_CMD_FILL_SD_WITH_RANDOM_CHARS:
            ...
            {
                CI_TickLocalPrintf
                    ("Fill SD card with random numbers - it runs very long, 1 GB ca. 4 minutes\r\n");

                UpdateStick20Command
                    (OUTPUT_CMD_STICK20_STATUS_BUSY_PROGRESSBAR, 0);

                // Disable all SD card access via USB
                SetSdUncryptedCardEnableState (FALSE);  // look for >>
                                                        // sd_mmc_mci_test_unit_only_local_access 
                                                        // = hard disabel
                SetSdEncryptedCardEnableState (FALSE);
                SetSdEncryptedHiddenState (FALSE);
                vTaskDelay (3000);

                AES_SetNewStorageKey ((u8 *) AES_DummyKey_au8); // Set dummy
                                                                // key

                /*
                 * if (STICK20_FILL_SD_CARD_WITH_RANDOM_CHARS_ALL_VOL ==
                 * HID_String_au8[0]) { SD_SecureEraseHoleCard (); } else {
                ...
                UpdateStick20Command
                    (OUTPUT_CMD_STICK20_STATUS_BUSY_PROGRESSBAR, 100);
                vTaskDelay (500);
                UpdateStick20Command (OUTPUT_CMD_STICK20_STATUS_OK, 0);
                vTaskDelay (500);
        ...
        case HTML_CMD_ENABLE_HIDDEN_AES_LUN:
            // SetSdUncryptedCardEnableState (FALSE); // look for >>
            // sd_mmc_mci_test_unit_only_local_access = hard disabel

            // Get w AES key for hidden volume - we use always the password
            // that was send
            memset (StorageKey_pu8, 0, 32);
            ret_u8 =
                GetHiddenVolumeKeyFromUserpassword ((u8 *) &
                                                    HID_String_au8[1],
                                                    StorageKey_pu8);
            switch (ret_u8)
            {
                case HIDDEN_VOLUME_OUTPUT_STATUS_OK:
                    SetSdEncryptedCardEnableState (FALSE);
                    SetSdEncryptedHiddenState (FALSE);
                    AES_SetNewStorageKey ((u8 *) AES_DummyKey_au8); // Set
                                                                    // dummy
                                                                    // key
                    ...
                    UpdateStick20Command (OUTPUT_CMD_STICK20_STATUS_OK, 0);
                    CI_TickLocalPrintf
                        ("Hidden volume - SD card hidden AES LUN enabled\r\n");
                    break;
                ...
                case HIDDEN_VOLUME_OUTPUT_STATUS_NO_USER_PASSWORD_UNLOCK:
                    CI_TickLocalPrintf
                        ("Hidden volume - no user password\r\n");
                    UpdateStick20Command
                        (OUTPUT_CMD_STICK20_STATUS_NO_USER_PASSWORD_UNLOCK,
                         0);
                    break;
                case HIDDEN_VOLUME_OUTPUT_STATUS_SMARTCARD_ERROR:
                    CI_TickLocalPrintf
                        ("Hidden volume - smartcard error\r\n");
                    UpdateStick20Command
                        (OUTPUT_CMD_STICK20_STATUS_SMARTCARD_ERROR, 0);
                    break;
                default:
                    break;
            }

            HID_NextPasswordIsHiddenPassword_u32 = FALSE;   // switch off
                                                            // redirection of 
                                                            // aes volume
                                                            // password
            break;
        ...
        case HTML_CMD_EXPORT_BINARY:
            ...
            {
                CI_TickLocalPrintf ("Export binary\r\n");
                // SetSdUncryptedCardEnableState (FALSE); // todo: Check
                // disable access, don't work here
    ...
    u8 CmdOk_u8 = TRUE;
            ...
            CI_TickLocalPrintf
                ("Generate new keys for hidden volume slots\r\n");
            ...
            CI_TickLocalPrintf
                ("Generate new keys for password safe store\r\n");
        ...
        case HTML_CMD_SEND_PASSWORD_MATRIX:
            ...
            UpdateStick20Command
                (OUTPUT_CMD_STICK20_STATUS_PASSWORD_MATRIX_READY, 0);
            break;
        ...
        case HTML_CMD_SEND_PASSWORD_MATRIX_SETUP:
            ...
            WriteMatrixColumsUserPWToUserPage (&HID_String_au8[1]); // Data
                                                                    // is in
                                                                    // ASCII
            UpdateStick20Command (OUTPUT_CMD_STICK20_STATUS_OK, 0);
            break;
        ...
        case HTML_CMD_WRITE_STATUS_DATA:
            ...
            // ((HID_StiHID_StringStatus_est*)HID_String_au8); // Changed
            UpdateStick20Command (OUTPUT_CMD_STICK20_STATUS_OK, 0);
            break;
        ...
        case HTML_CMD_SEND_PASSWORD:
            ...
            switch (HID_String_au8[0])
            {
                case 'P':
                    ...
                    strncpy ((char *) HID_UserPassword_au8,
                             (char *) &HID_String_au8[1], HID_PASSWORD_LEN);
                    break;
                case 'A':
                    ret_u8 = IW_SendToSC_PW3 (&HID_String_au8[1]);
                    HID_AdminPasswordEnabled_u32 = ret_u8;
                    strncpy ((char *) HID_AdminPassword_au8,
                             (char *) &HID_String_au8[1], HID_PASSWORD_LEN);
                    break;
            }
                ...
                case 'P':
                    if (TRUE == HID_UserPasswordEnabled_u32)
                    {
                        ret_u8 =
                            ...
                            (HID_UserPassword_au8, &HID_String_au8[1]);
                    }
                    else
                    {
                        CI_TickLocalPrintf
                ...
                case 'A':
                    if (TRUE == HID_AdminPasswordEnabled_u32)
                    {
                        ret_u8 =
                            ...
                            (HID_AdminPassword_au8, &HID_String_au8[1]);
                    }
                    else
                    {
                        CI_TickLocalPrintf
                            ("** Admin password not present ***\r\n");
                    }
                    break;
            }
        ...
        case HTML_CMD_SEND_STARTUP:
             ...
             * CI_TickLocalPrintf ("Get
             * STICK2_COUNT\END_PASSWORD_RETRY_COUNT\r\n");
             * 
             * // Stick20HIDInitSendConfiguration (STICK20_SEND_STARTUP);
             * UpdateStick20Command (OUTPUT_CMD_STICK20_STATUS_OK,0); break; 
             */
        case HTML_CMD_HIDDEN_VOLUME_SETUP:
            CI_TickLocalPrintf ("Get HTML_CMD_HIDDEN_VOLUME_SETUP\r\n");
            ret_u8 =
                SetupUpHiddenVolumeSlot ((HiddenVolumeSetup_tst *) &
                                         HID_String_au8[0]);
            switch (ret_u8)
            {
                case HIDDEN_VOLUME_OUTPUT_STATUS_OK:
        ...
        case HTML_CMD_CLEAR_STICK_KEYS_NOT_INITIATED:
            CI_TickLocalPrintf
                ("Get HTML_CMD_CLEAR_STICK_KEYS_NOT_INITIATED\r\n");
        ...
        case HTML_CMD_CLEAR_LOCK_STICK_HARDWARE:
            ...
            if (TRUE == IW_SendToSC_PW3 (&HID_String_au8[1]))
            {
                ...
                flashc_activate_security_bit ();    // Debugging disabled,
                                                    // only chip erase works
                                                    // (bootloader is save) , 
                                                    // AES storage keys and
                                                    // setup are erased
                UpdateStick20Command (OUTPUT_CMD_STICK20_STATUS_OK, 0);
            }
            else
...
u32 Ret_u32 = FALSE;
    ...
    if (TRUE == Ret_u32)
    {
        ...
        Status_st->ActiveSmartCardID_u32 =
            Status_st->ActiveSmartCardID_u32 << 8;
...
cid_t *cid;
    ...
    {
        if (FALSE ==
            LA_OpenPGP_V20_Test_SendUserPW2 ((unsigned char *) "123456"))
        {
            CI_TickLocalPrintf
                ("GetProductionInfos: Intial password is not activ\r\n");
    ...
    {
        CI_TickLocalPrintf
            ("GetProductionInfos: Password retry count is not 3 : %d\r\n",
             SC_Status_st.UserPwRetryCount);
        Infos_st->SC_AdminPwRetryCount = 88;    // Marker for wrong pw retry
                                                // count
        return;
    }
