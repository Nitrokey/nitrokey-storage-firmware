...
extern "C"
{
     ...
     * 
     * It will generally be sensible to use tables to compute finite field
     * multiplies and inverses but where memory is scarse this code might
     * sometimes be better. But it only has effect during initialisation so
     * its pretty unimportant in overall terms. 
     */

    /*
     * return 2 ^ (n - 1) where n is the bit number of the highest bit set in 
     * x with x in the range 1 < x < 0x00000200.  This form is used so that
...
#define inv_affine(x) \
    ...
    AES_RETURN aes_init (void)
    {
        ...
        if (init)
            return EXIT_SUCCESS;
        /*
         * log and power tables for GF(2^8) finite field with WPOLY as
         * modular polynomial - the simplest primitive root is 0x03, used
