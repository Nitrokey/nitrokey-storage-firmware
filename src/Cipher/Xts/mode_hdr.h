...
/*
 * This define sets the units in which buffers are processed.  This code can
 * provide significant speed gains if buffers can be processed in 32 or 64
 * bit chunks rather than in bytes.  This define sets the units in which
 * buffers will be accessed if possible 
 */
 ...
 * using 'defines'.  But the latter approach will typically generate a LOT of 
...
#define rep3_d16(f,r,x,y) f(15,r,x,y); f(14,r,x,y); f(13,r,x,y); f(12,r,x,y); \
    ...
    {
        return bswap_32 ((uint_32t) (x >> 32)) | ((uint_64t)
                                                  bswap_32 ((uint_32t) x) <<
                                                  32);
    }
#endif
     ...
     * overlap, those for byte order revesal must either not overlap or must
     * be identical 
     */
...
#define f_xor(n,r,p,q)  r[n] = p[n] ^ q[n]
    ...
    /*
     * platform byte order to big or little endian order for 16, 32 and 64
     * bit variables 
     */
