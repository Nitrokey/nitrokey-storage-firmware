...
char *des_name[] =
    { "EKY ", "TKY ", "LBA ", "PTX ", "CTX ", "SLN ", "VEC ", "END", "REM ",
"RPT " };
enum des_type des_m[] =
...
void do_test (const char *in_dir, const char *out_dir, const char *name,
              int gen_flag)
{
    char tvi_name[64], tvo_name[64], line[80], *lp;
    FILE *inf = 0, *outf = 0;
    unsigned char tky[32], eky[64], sln[16], lba[16], ptx[4096], ctx[4096],
        res[4096], *p;
    int tky_len, eky_len, sln_len, lba_len, ptx_len, ctx_len, ptx_rpt,
        rpt_cnt, vec_no, *cnt, *rpt = &rpt_cnt, err, i;
    ...
    if (strrchr (tvi_name, '\\') != lp - 1
        && strrchr (tvi_name, '/') != lp - 1)
    {
        *lp++ = '\\';
        *lp = '\0';
    }
    ...
    if (gen_flag)
    {
        ...
        if (strrchr (tvo_name, '\\') != lp - 1
            && strrchr (tvo_name, '/') != lp - 1)
        {
            *lp++ = '\\';
            *lp = '\0';
        }
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            if (strlen (line) < 4)
            {
                tky_len = eky_len = sln_len = lba_len = ptx_len =
                    ctx_len = rpt_cnt = 0;
                ptx_rpt = 1;
                if (gen_flag)
                    fprintf (outf, "\n");
            ...
            while (i--)
            {
                ...
                xts_encrypt_sector (res, bytes_to_number (lba, lba_len),
                                    ptx_len, ectx);
                if (memcmp (ctx, res, ptx_len))
                {
                    printf ("\n\tencrypt error on test number %i", vec_no),
                        err++;
                    break;
                }
                xts_decrypt_sector (res, bytes_to_number (lba, lba_len),
                                    ptx_len, dctx);
                if (memcmp (ptx, res, ptx_len))
                {
                    printf ("\n\tdecrypt error on test number %i", vec_no),
                        err++;
                    break;
                }
            }
