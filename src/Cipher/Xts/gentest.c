...
/*
 * The following definitions describe the different lines that can occur in
 * test vector files.  Each line starts with a four character line designator 
 * in which the last character is a space (which must be present).  Some
 * lines are commands (e.g GEN means generate a test vector) while others
 * specify the values of inputs (e.g. KEY gives the value of the key to be
 * used) while others (e.g. CTX and TAG) give the expected value of outputs.
 * In the list of sequence designators, inputs follow outputs and inputs are
 * listed with the values that vary slowly between suquential test vectors
 * before those that vary more rapidly. 
 */
enum des_type
{ EKY = 0, TKY, LBA, PTX, CTX,  /* sequence values */
    VEC = 16, GEN, END, REM, SLN, NUL, ERR
};  /* non-sequence values */

/*
 * Input sequence designators are listed first with the most slowly varying
 * values earlier in the list. Then output values are listed, followed by
 * other values such as commands and aliases (e.g. IV is an alias for NCE). 
 */
char *des_name[] = { "EKY", "TKY", "LBA", "PTX", "CTX", "VEC", "GEN", "END" };

enum des_type des_m[] =
{ EKY, TKY, LBA, PTX, CTX, VEC, GEN, END };

/*
 * Find if a line starts with a recognised line designator and return its
 * descriptor if it does or ERR if not. 
 */
...
static unsigned char key[16] =
    { 3, 1, 4, 1, 5, 9, 2, 6, 5, 3, 5, 8, 2, 3, 8, 4 };
...
/*
 * This structure accumulates the character strings for each specific line
 * designator in a test vector template. The values are accumulated in the
 * dynamically allocated C string pointed to by rp.  It also records the
 * position in this string while p;arsing it (pos) and numerical values
 * obtained as a result (cnt[6]). The number of values obtained, up to five,
 * is set in cnt[0] 
 */
typedef struct
{
...
void add_to_record (v_rec * v, char *c)
{
    ...
    char *p;
         ...
         */
        v->len =
            (v->rp ? (unsigned int) strlen (v->rp) : 0) +
            (unsigned int) strlen (c) + 80;
        p = malloc (v->len);
        *p = '\0';
        /*
 ...
 * numerical values within brackets.  This routine parses the latter bracket
 * terms and puts the number of values obtained in cnt[0] and the values in
 * cnt[1] .. cnt[5]. Numbers are hexadecimal unless they are preceeded by a
 * '#', in which case they are decimal. 
 */
...
int gen_vectors (char *in_file_name, char *out_file_name)
{
    ...
    struct tm *now;
            ...
            time (&secs);
            now = localtime (&secs);
            fprintf (out_file, "REM Produced by GENTEST on %s",
                     asctime (now));
        }
                ...
                while (get_value (rec + 1, tky, &tky_len, 0))
                {
                    while (get_value (rec + 2, lba, &lba_len, 0))
                    {
                        while (get_value (rec + 3, ptx, &ptx_len, ctx[0]))
                        {
                            ...
                            if (xts_encrypt_key
                                (eky, eky_len + tky_len,
                                 enc_ctx) != EXIT_SUCCESS)
                                goto exit1;
                            if (xts_decrypt_key
                                (eky, eky_len + tky_len,
                                 dec_ctx) != EXIT_SUCCESS)
                                goto exit1;
                            memcpy (ctx[0], ptx, ptx_len);
                            if (xts_encrypt_sector
                                (ctx[0], bytes_to_number (lba, lba_len),
                                 ptx_len, enc_ctx) != EXIT_SUCCESS)
                                goto exit1;
                            memcpy (ctx[1], ctx[0], ptx_len);
                            if (xts_decrypt_sector
                                (ctx[1], bytes_to_number (lba, lba_len),
                                 ptx_len, dec_ctx) != EXIT_SUCCESS)
                                goto exit1;
                            ...
                            fprintf (out_file, "\nVEC %d", (int) vec_no++);
                            hex_out (out_file, des_name[EKY & 15], eky,
                                     eky_len);
                            hex_out (out_file, des_name[TKY & 15], tky,
                                     tky_len);
                            hex_out (out_file, des_name[LBA & 15], lba,
                                     lba_len);
                            hex_out (out_file, des_name[PTX & 15], ptx,
                                     ptx_len);
                            hex_out (out_file, des_name[CTX & 15], ctx[0],
                                     ptx_len);
                            fprintf (out_file, "\n");
                        }
                        restart (rec + 3);
