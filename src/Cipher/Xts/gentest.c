...
/* The following definitions describe the different lines that can occur in test vector files.  Each line starts with a four character line
   designator in which the last character is a space (which must be present).  Some lines are commands (e.g GEN means generate a test vector) while
   others specify the values of inputs (e.g. KEY gives the value of the key to be used) while others (e.g. CTX and TAG) give the expected value of
   outputs. In the list of sequence designators, inputs follow outputs and inputs are listed with the values that vary slowly between suquential
   test vectors before those that vary more rapidly. */
enum des_type
{ EKY = 0, TKY, LBA, PTX, CTX,  /* sequence values */
    VEC = 16, GEN, END, REM, SLN, NUL, ERR
};  /* non-sequence values */

/* Input sequence designators are listed first with the most slowly varying values earlier in the list. Then output values are listed, followed by
...
/* Find if a line starts with a recognised line designator and return its descriptor if it does or ERR if not. */
...
/* This structure accumulates the character strings for each specific line designator in a test vector template. The values are accumulated in the
   dynamically allocated C string pointed to by rp.  It also records the position in this string while p;arsing it (pos) and numerical values
   obtained as a result (cnt[6]). The number of values obtained, up to five, is set in cnt[0] */
typedef struct
{
    char* rp;
    unsigned int len;
...
void add_to_record (v_rec * v, char* c)
{
    ...
    if (*(v->rp) && *c != ',' && v->rp[strlen (v->rp) - 1] != ',' && (!isxdigit (v->rp[strlen (v->rp) - 1]) || !isxdigit (*c)))
        strcat (v->rp, ",");
    /* add the new content */
    strcat (v->rp, c);
}

/* input strings are comma separated values which are either hexadecimal sequences or 'bracket terms' consisting of up to five comma separated
   numerical values within brackets.  This routine parses the latter bracket terms and puts the number of values obtained in cnt[0] and the values
   in cnt[1] .. cnt[5]. Numbers are hexadecimal unless they are preceeded by a '#', in which case they are decimal. */
                ...
                while (get_value (rec + 1, tky, &tky_len, 0))
                {
                    while (get_value (rec + 2, lba, &lba_len, 0))
                    {
                        while (get_value (rec + 3, ptx, &ptx_len, ctx[0]))
                        {
                            ...
                            if (xts_encrypt_sector (ctx[0], bytes_to_number (lba, lba_len), ptx_len, enc_ctx) != EXIT_SUCCESS)
                                goto exit1;
                            memcpy (ctx[1], ctx[0], ptx_len);
                            if (xts_decrypt_sector (ctx[1], bytes_to_number (lba, lba_len), ptx_len, dec_ctx) != EXIT_SUCCESS)
                                goto exit1;
