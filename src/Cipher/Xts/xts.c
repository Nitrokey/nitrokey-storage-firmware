...
void gf_mulx (void *x)
{
    ...
    while (--i)
        ((uint_8t *) x)[i] =
            (((uint_8t *) x)[i] << 1) | (((uint_8t *) x)[i - 1] & 0x80 ? 1 :
                                         0);
...
#define GF_XOR   li_64(0000000000000087)
    ...
    UPTR_CAST (x, 64)[1] =
        (UPTR_CAST (x, 64)[1] << 1) | (UPTR_CAST (x, 64)[0] & GF_MASK ? 1 :
                                       0);
...
#define GF_XOR   li_32(00000087)
    ...
    UPTR_CAST (x, 32)[3] =
        (UPTR_CAST (x, 32)[3] << 1) | (UPTR_CAST (x, 32)[2] & GF_MASK ? 1 :
                                       0);
    UPTR_CAST (x, 32)[2] =
        (UPTR_CAST (x, 32)[2] << 1) | (UPTR_CAST (x, 32)[1] & GF_MASK ? 1 :
                                       0);
    UPTR_CAST (x, 32)[1] =
        (UPTR_CAST (x, 32)[1] << 1) | (UPTR_CAST (x, 32)[0] & GF_MASK ? 1 :
                                       0);
...
#define GF_XOR   li_64(8700000000000000)
    ...
    UPTR_CAST (x, 64)[1] = ((UPTR_CAST (x, 64)[1] << 1) & ~MASK_01)
        | (((UPTR_CAST (x, 64)[1] >> 15) | (UPTR_CAST (x, 64)[0] << 49)) &
           MASK_01);
    UPTR_CAST (x, 64)[0] =
        (((UPTR_CAST (x, 64)[0] << 1) & ~MASK_01) |
...
#define GF_XOR   li_32(87000000)
    ...
    UPTR_CAST (x, 32)[3] = ((UPTR_CAST (x, 32)[3] << 1) & ~MASK_01)
        | (((UPTR_CAST (x, 32)[3] >> 15) | (UPTR_CAST (x, 32)[2] << 17)) &
           MASK_01);
    UPTR_CAST (x, 32)[2] =
        ((UPTR_CAST (x, 32)[2] << 1) & ~MASK_01) |
        (((UPTR_CAST (x, 32)[2] >> 15) | (UPTR_CAST (x, 32)[1] << 17)) &
         MASK_01);
    UPTR_CAST (x, 32)[1] =
        ((UPTR_CAST (x, 32)[1] << 1) & ~MASK_01) |
        (((UPTR_CAST (x, 32)[1] >> 15) | (UPTR_CAST (x, 32)[0] << 17)) &
         MASK_01);
    UPTR_CAST (x, 32)[0] =
        (((UPTR_CAST (x, 32)[0] << 1) & ~MASK_01) |
...
INT_RETURN xts_encrypt_key (const unsigned char key[], int key_len,
                            xts_encrypt_ctx ctx[1])
{
    int aes_klen_by;
    ...
    return aes_encrypt_key (key, aes_klen_by, ctx->enc_ctx) == EXIT_SUCCESS &&
        aes_encrypt_key (key + aes_klen_by, aes_klen_by,
                         ctx->twk_ctx) ==
        EXIT_SUCCESS ? EXIT_SUCCESS : EXIT_FAILURE;
}

INT_RETURN xts_decrypt_key (const unsigned char key[], int key_len,
                            xts_decrypt_ctx ctx[1])
{
int aes_klen_by;
...
INT_RETURN xts_encrypt_sector (unsigned char sector[],
                               lba_type sector_address,
                               unsigned int sector_len,
                               const xts_encrypt_ctx ctx[1])
{
buf_type hh;
uint_8t *pos = sector, *hi = sector + sector_len;

xor_function f_ptr =
    (!ALIGN_OFFSET (sector, UNIT_BITS >> 3) ? xor_block_aligned : xor_block);
...
INT_RETURN xts_decrypt_sector (unsigned char sector[],
                               lba_type sector_address,
                               unsigned int sector_len,
                               const xts_decrypt_ctx ctx[1])
{
buf_type hh, hh2;
uint_8t *pos = sector, *hi = sector + sector_len;
