...
unsigned char* buf,* p;
    ...
    if ((buf[0] & 0x80) != 0)
    {
        ...
        for (i = 0; ssl->ciphers[i] != 0; i++)
        {
            ...
            {
                if (p[0] == 0 && p[1] == 0 && p[2] == ssl->ciphers[i])
                    goto have_cipher;
            }
        }
    }
...
unsigned char* buf,* p;
    ...
    *p++ = (unsigned char) ssl->session->length;
    ...
    if (ssl->s_get == NULL || ssl->s_get (ssl) != 0)
    {
        ...
        for (i = 0; i < n; i++)
            ssl->session->id[i] = (unsigned char) ssl->f_rng (ssl->p_rng);
    }
    else
    {
        /*
...
sha1_context sha1;
    ...
    if (ssl->session->cipher != SSL_EDH_RSA_DES_168_SHA &&
        ssl->session->cipher != SSL_EDH_RSA_AES_256_SHA && ssl->session->cipher != SSL_EDH_RSA_CAMELLIA_256_SHA)
    {
    ...
    ret = rsa_pkcs1_sign (ssl->rsa_key, RSA_PRIVATE, SIG_RSA_RAW, 36, hash, ssl->out_msg + 6 + n);
    if (ret != 0)
    {
...
int ret, i, n;
    ...
    else
    {
        ...
        {
            i += 2;
            if (ssl->in_msg[4] != ((n >> 8) & 0xFF) || ssl->in_msg[5] != ((n) & 0xFF))
            {
                SSL_DEBUG_MSG (1, ("bad client key exchange message"));
                return (POLARSSL_ERR_SSL_BAD_HS_CLIENT_KEY_EXCHANGE);
            }
        ...
        ret = rsa_pkcs1_decrypt (ssl->rsa_key, RSA_PRIVATE, &ssl->pmslen, ssl->in_msg + i, ssl->premaster, sizeof (ssl->premaster));

        if (ret != 0 || ssl->pmslen != 48 || ssl->premaster[0] != ssl->max_major_ver || ssl->premaster[1] != ssl->max_minor_ver)
        {
            SSL_DEBUG_MSG (1, ("bad client key exchange message"));
...
unsigned char hash[36];
    ...
    ret = rsa_pkcs1_verify (&ssl->peer_cert->rsa, RSA_PUBLIC, SIG_RSA_RAW, 36, hash, ssl->in_msg + 6);
    if (ret != 0)
    {
