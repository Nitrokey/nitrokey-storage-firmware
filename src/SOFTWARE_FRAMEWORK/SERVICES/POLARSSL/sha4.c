...
static unsigned char sha4_hmac_test_buf[7][153] = {
    ...
    {"This is a test using a larger than block-size key "
     "and a larger than block-size data. The key needs to " "be hashed before being used by the HMAC algorithm."}
};
...
int sha4_self_test (int verbose)
{
    ...
    for (i = 0; i < 14; i++)
    {
        ...
        if (j == 5 || j == 6)
        {
            ...
            sha4_hmac_starts (&ctx, sha4_hmac_test_key[j], sha4_hmac_test_keylen[j], k);

        sha4_hmac_update (&ctx, sha4_hmac_test_buf[j], sha4_hmac_test_buflen[j]);
