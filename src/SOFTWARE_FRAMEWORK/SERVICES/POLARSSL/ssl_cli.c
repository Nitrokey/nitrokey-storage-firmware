...
unsigned char* buf;
    ...
    if (ssl->in_hslen < 42 || buf[0] != SSL_HS_SERVER_HELLO || buf[4] != SSL_MAJOR_VERSION_3)
    {
        SSL_DEBUG_MSG (1, ("bad server hello message"));
     ...
     */
    if (ssl->resume == 0 || n == 0 || ssl->session->cipher != i || ssl->session->length != n || memcmp (ssl->session->id, buf + 39, n) != 0)
    {
        ssl->state++;
        ssl->resume = 0;
...
sha1_context sha1;
    ...
    if ((ret = rsa_pkcs1_verify (&ssl->peer_cert->rsa, RSA_PUBLIC, SIG_RSA_RAW, 36, hash, p)) != 0)
    {
...
unsigned char hash[36];
    ...
    if ((ret = rsa_pkcs1_sign (ssl->rsa_key, RSA_PRIVATE, SIG_RSA_RAW, 36, hash, ssl->out_msg + 6)) != 0)
    {
