...
static int x509_get_alg (unsigned char **p,
                         unsigned char *end, x509_buf * alg)
{
    int ret, len;
...
static int x509_get_name (unsigned char **p,
                          unsigned char *end, x509_name * cur)
{
    int ret, len;
    unsigned char *end2;
    x509_buf *oid;
...
static int x509_get_UTCTime (unsigned char **p,
                             unsigned char *end, x509_time * time)
{
    int ret, len;
    char date[64];
...
static int x509_get_sig (unsigned char **p,
                         unsigned char *end, x509_buf * sig)
{
    int ret, len;
...
static int x509_get_ext (unsigned char **p,
                         unsigned char *end, x509_buf * ext)
{
    int ret, len;
...
static int x509_get_entries (unsigned char **p,
                             unsigned char *end, x509_crl_entry * entry)
{
    int ret, entry_len;
    ...
    int len2;
        ...
        if ((ret =
             x509_get_UTCTime (p, end, &cur_entry->revocation_date)) != 0)
            return (ret);
...
time_t tt;
    ...
    if (lt->tm_year == to->year - 1900 &&
        lt->tm_mon == to->mon - 1 && lt->tm_mday > to->day)
        return (1);
