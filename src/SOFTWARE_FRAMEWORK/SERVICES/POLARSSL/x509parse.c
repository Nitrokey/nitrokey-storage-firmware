...
static int x509_get_version (unsigned char** p, unsigned char* end, int* ver)
{
    ...
    if (*p != end)
        return (POLARSSL_ERR_X509_CERT_INVALID_VERSION | POLARSSL_ERR_ASN1_LENGTH_MISMATCH);
...
static int x509_get_serial (unsigned char** p, unsigned char* end, x509_buf * serial)
{
    ...
    if ((end - *p) < 1)
        return (POLARSSL_ERR_X509_CERT_INVALID_SERIAL | POLARSSL_ERR_ASN1_OUT_OF_DATA);

    if (**p != (ASN1_CONTEXT_SPECIFIC | ASN1_PRIMITIVE | 2) && **p != ASN1_INTEGER)
        return (POLARSSL_ERR_X509_CERT_INVALID_SERIAL | POLARSSL_ERR_ASN1_UNEXPECTED_TAG);
...
static int x509_get_alg (unsigned char** p, unsigned char* end, x509_buf * alg)
{
    ...
    *p += alg->len;
    ...
    if (*p != end)
        return (POLARSSL_ERR_X509_CERT_INVALID_ALG | POLARSSL_ERR_ASN1_LENGTH_MISMATCH);
...
static int x509_get_name (unsigned char** p, unsigned char* end, x509_name * cur)
{
    ...
    if ((ret = asn1_get_tag (p, end, &len, ASN1_CONSTRUCTED | ASN1_SET)) != 0)
    ...
    if (*p + len != end)
        return (POLARSSL_ERR_X509_CERT_INVALID_NAME | POLARSSL_ERR_ASN1_LENGTH_MISMATCH);
    ...
    *p += oid->len;
    ...
    if ((end - *p) < 1)
        return (POLARSSL_ERR_X509_CERT_INVALID_NAME | POLARSSL_ERR_ASN1_OUT_OF_DATA);

    if (**p != ASN1_BMP_STRING && **p != ASN1_UTF8_STRING &&
        **p != ASN1_T61_STRING && **p != ASN1_PRINTABLE_STRING && **p != ASN1_IA5_STRING && **p != ASN1_UNIVERSAL_STRING)
        return (POLARSSL_ERR_X509_CERT_INVALID_NAME | POLARSSL_ERR_ASN1_UNEXPECTED_TAG);
 ...
 */
static int x509_get_UTCTime (unsigned char** p, unsigned char* end, x509_time * time)
{
    int ret, len;
    char date[64];
    ...
    if (sscanf (date, "%2d%2d%2d%2d%2d%2d", &time->year, &time->mon, &time->day, &time->hour, &time->min, &time->sec) < 5)
...
static int x509_get_dates (unsigned char** p, unsigned char* end, x509_time * from, x509_time * to)
{
    ...
    if (*p != end)
        return (POLARSSL_ERR_X509_CERT_INVALID_DATE | POLARSSL_ERR_ASN1_LENGTH_MISMATCH);
...
static int x509_get_pubkey (unsigned char** p, unsigned char* end, x509_buf * pk_alg_oid, mpi * N, mpi * E)
{
    ...
    if ((end - *p) < 1)
        return (POLARSSL_ERR_X509_CERT_INVALID_PUBKEY | POLARSSL_ERR_ASN1_OUT_OF_DATA);
     ...
     *  }
     */
    if ((ret = asn1_get_tag (p, end2, &len, ASN1_CONSTRUCTED | ASN1_SEQUENCE)) != 0)
        return (POLARSSL_ERR_X509_CERT_INVALID_PUBKEY | ret);

    if (*p + len != end2)
        return (POLARSSL_ERR_X509_CERT_INVALID_PUBKEY | POLARSSL_ERR_ASN1_LENGTH_MISMATCH);
...
static int x509_get_uid (unsigned char** p, unsigned char* end, x509_buf * uid, int n)
{
    ...
    if ((ret = asn1_get_tag (p, end, &uid->len, ASN1_CONTEXT_SPECIFIC | ASN1_CONSTRUCTED | n)) != 0)
    {
        if (ret == POLARSSL_ERR_ASN1_UNEXPECTED_TAG)
            return (0);
 ...
 */
static int x509_get_ext (unsigned char** p, unsigned char* end, x509_buf * ext)
{
    int ret, len;
    ...
    if ((ret = asn1_get_tag (p, end, &ext->len, ASN1_CONTEXT_SPECIFIC | ASN1_CONSTRUCTED | 3)) != 0)
        return (ret);
    ...
    if (end != *p + len)
        return (POLARSSL_ERR_X509_CERT_INVALID_EXTENSIONS | POLARSSL_ERR_ASN1_LENGTH_MISMATCH);
 ...
 */
static int x509_get_crl_ext (unsigned char** p, unsigned char* end, x509_buf * ext)
{
    int ret, len;
 ...
 */
static int x509_get_crt_ext (unsigned char** p, unsigned char* end, x509_buf * ext, int* ca_istrue, int* max_pathlen)
{
    int ret, len;
    int is_critical = 1;
    int is_cacert = 0;
    ...
    while (*p < end)
    {
        ...
        *p += 5;
        ...
        if ((ret = asn1_get_bool (p, end_ext_data, &is_critical)) != 0 && (ret != POLARSSL_ERR_ASN1_UNEXPECTED_TAG))
            return (POLARSSL_ERR_X509_CERT_INVALID_EXTENSIONS | ret);

        if ((ret = asn1_get_tag (p, end_ext_data, &len, ASN1_OCTET_STRING)) != 0)
            return (POLARSSL_ERR_X509_CERT_INVALID_EXTENSIONS | ret);
        ...
        if ((ret = asn1_get_tag (p, end_ext_octet, &len, ASN1_CONSTRUCTED | ASN1_SEQUENCE)) != 0)
            return (POLARSSL_ERR_X509_CERT_INVALID_EXTENSIONS | ret);
 ...
 * X.509 CRL Entries
 */
static int x509_get_entries (unsigned char** p, unsigned char* end, x509_crl_entry * entry)
{
    int ret, entry_len;
    ...
    if ((ret = asn1_get_tag (p, end, &entry_len, ASN1_SEQUENCE | ASN1_CONSTRUCTED)) != 0)
    {
        if (ret == POLARSSL_ERR_ASN1_UNEXPECTED_TAG)
            return (0);
    ...
    int len2;
        ...
        if ((ret = asn1_get_tag (p, end, &len2, ASN1_SEQUENCE | ASN1_CONSTRUCTED)) != 0)
        {
            return (ret);
        }
...
int x509parse_crt (x509_cert * chain, unsigned char* buf, int buflen)
{
    ...
    if ((ret = x509_get_version (&p, end, &crt->version)) != 0 ||
        (ret = x509_get_serial (&p, end, &crt->serial)) != 0 || (ret = x509_get_alg (&p, end, &crt->sig_oid1)) != 0)
    {
        x509_free (crt);
        return (ret);
    }
    ...
    if (crt->sig_oid1.p[8] < 2 || (crt->sig_oid1.p[8] > 5 && crt->sig_oid1.p[8] < 11) || crt->sig_oid1.p[8] > 14)
    {
        x509_free (crt);
        return (POLARSSL_ERR_X509_CERT_UNKNOWN_SIG_ALG);
    }
...
int x509parse_crl (x509_crl * chain, unsigned char* buf, int buflen)
{
     ...
     * signature            AlgorithmIdentifier
     */
    if ((ret = x509_get_version (&p, end, &crl->version)) != 0 || (ret = x509_get_alg (&p, end, &crl->sig_oid1)) != 0)
    {
        x509_crl_free (crl);
        return (ret);
    }
    ...
    if (crl->sig_oid1.len != 9 || memcmp (crl->sig_oid1.p, OID_PKCS1, 8) != 0)
    {
        x509_crl_free (crl);
        return (POLARSSL_ERR_X509_CERT_UNKNOWN_SIG_ALG);
    }

    if (crl->sig_oid1.p[8] < 2 || (crl->sig_oid1.p[8] > 5 && crl->sig_oid1.p[8] < 11) || crl->sig_oid1.p[8] > 14)
    {
        x509_crl_free (crl);
        return (POLARSSL_ERR_X509_CERT_UNKNOWN_SIG_ALG);
    }
    ...
    if ((ret = x509_get_UTCTime (&p, end, &crl->next_update)) != 0)
    {
        if (ret != (POLARSSL_ERR_X509_CERT_INVALID_DATE |
                    POLARSSL_ERR_ASN1_UNEXPECTED_TAG) && ret != (POLARSSL_ERR_X509_CERT_INVALID_DATE | POLARSSL_ERR_ASN1_OUT_OF_DATA))
        {
            x509_crl_free (crl);
            return (ret);
        }
...
int x509parse_key (rsa_context * rsa, unsigned char* buf, int buflen, unsigned char* pwd, int pwdlen)
{
    ...
    s1 = (unsigned char *) strstr ((char *) buf, "-----BEGIN RSA PRIVATE KEY-----");

    if (s1 != NULL)
    {
        s2 = (unsigned char *) strstr ((char *) buf, "-----END RSA PRIVATE KEY-----");
    ...
    if ((ret = asn1_get_mpi (&p, end, &rsa->N)) != 0 ||
        ...
        (ret = asn1_get_mpi (&p, end, &rsa->DQ)) != 0 || (ret = asn1_get_mpi (&p, end, &rsa->QP)) != 0)
    {
        if (s1 != NULL)
            free (buf);
    ...
    if (p != end)
    {
        ...
        rsa_free (rsa);
        return (POLARSSL_ERR_X509_KEY_INVALID_FORMAT | POLARSSL_ERR_ASN1_LENGTH_MISMATCH);
    }
...
int x509parse_keyfile (rsa_context * rsa, char* path, char* pwd)
{
    ...
    if (pwd == NULL)
        ...
        ret = x509parse_key (rsa, buf, (int) n, (unsigned char *) pwd, strlen (pwd));
...
int x509parse_cert_info (char* buf, size_t size, char* prefix, x509_cert * crt)
{
    ...
    nr = (crt->serial.len <= 32) ? crt->serial.len : 32;
    ...
    ret = snprintf (p, n, "\n%sissued  on    : "
                    ...
                    crt->valid_from.year, crt->valid_from.mon, crt->valid_from.day, crt->valid_from.hour, crt->valid_from.min, crt->valid_from.sec);
    SAFE_SNPRINTF ();

    ret = snprintf (p, n, "\n%sexpires on    : "
                    "%04d-%02d-%02d %02d:%02d:%02d", prefix,
                    crt->valid_to.year, crt->valid_to.mon, crt->valid_to.day, crt->valid_to.hour, crt->valid_to.min, crt->valid_to.sec);
    SAFE_SNPRINTF ();
...
int x509parse_crl_info (char* buf, size_t size, char* prefix, x509_crl * crl)
{
    ...
    while (entry != NULL && entry->raw.len != 0)
    {
        ...
        nr = (entry->serial.len <= 32) ? entry->serial.len : 32;
        ...
        ret = snprintf (p, n, " revocation date: "
                        ...
                        entry->revocation_date.day, entry->revocation_date.hour, entry->revocation_date.min, entry->revocation_date.sec);
        SAFE_SNPRINTF ();
...
time_t tt;
    ...
    if (lt->tm_year == to->year - 1900 && lt->tm_mon > to->mon - 1)
        return (1);

    if (lt->tm_year == to->year - 1900 && lt->tm_mon == to->mon - 1 && lt->tm_mday > to->day)
        return (1);
...
int x509parse_verify (x509_cert * crt, x509_cert * trust_ca, x509_crl * ca_crl, char* cn, int* flags)
{
    ...
    *flags |= BADCERT_NOT_TRUSTED;
    ...
    {
        if (cur->ca_istrue == 0 ||
            crt->issuer_raw.len != cur->subject_raw.len || memcmp (crt->issuer_raw.p, cur->subject_raw.p, crt->issuer_raw.len) != 0)
        {
            cur = cur->next;
            continue;
        }
    ...
    {
        if (crt->issuer_raw.len != trust_ca->subject_raw.len || memcmp (crt->issuer_raw.p, trust_ca->subject_raw.p, crt->issuer_raw.len) != 0)
        {
    ...
    while (trust_ca != NULL && ca_crl != NULL && ca_crl->version != 0)
    {
        if (ca_crl->issuer_raw.len != trust_ca->subject_raw.len ||
            memcmp (ca_crl->issuer_raw.p, trust_ca->subject_raw.p, ca_crl->issuer_raw.len) != 0)
        {
            ca_crl = ca_crl->next;
            continue;
        }
        ...
        if (!rsa_pkcs1_verify (&trust_ca->rsa, RSA_PUBLIC, hash_id, 0, hash, ca_crl->sig.p) == 0)
        {
            /*
             * CRL is not trusted
             */
...
int x509_self_test (int verbose)
{
    ...
    ret = x509parse_crt (&clicert, (unsigned char *) test_cli_crt, strlen (test_cli_crt));
    if (ret != 0)
    {
        if (verbose != 0)
            printf ("failed\n");
    ...
    ret = x509parse_crt (&cacert, (unsigned char *) test_ca_crt, strlen (test_ca_crt));
    if (ret != 0)
    {
        if (verbose != 0)
            printf ("failed\n");
    ...
    if ((ret = x509parse_key (&rsa, (unsigned char *) test_ca_key, i, (unsigned char *) test_ca_pwd, j)) != 0)
    {
        if (verbose != 0)
            printf ("failed\n");
