...
inline rsa_context* d2i_RSA_PUBKEY (void* ignore, unsigned char** bufptr, int len)
{
...
rsa_context* rsa;
    ...
    if ((len == 94 &&
        ...
        (len == 162 && mpi_read_binary (&rsa->N, &buffer[29], 128) == 0) && mpi_read_binary (&rsa->E, &buffer[159], 3) == 0)
    {
        /*
