 ...
 * ! \file
 * *********************************************************************
 * \brief FreeRTOS port source for AVR32 UC3. - Compiler: GNU GCC for AVR32
 * - Supported devices: All AVR32 devices can be used. - AppNote: \author
 ...
 */
#define portINITIAL_SR            ( ( portSTACK_TYPE ) 0x00400000 ) /* AVR32
                                                                     * :
                                                                     * [:2:M0]=001 
                                                                     * I1M=0
                                                                     * I0M=0, 
...
/*
 * The cooperative scheduler requires a normal IRQ service routine to simply
...
static void vTick (void)
{
    ...
    /*
     * Because FreeRTOS is not supposed to run with nested interrupts, put
     * all OS calls in a critical section . 
     */
    portENTER_CRITICAL ();
    vTaskIncrementTick ();
    portEXIT_CRITICAL ();
 ...
 * different optimisation levels.  The interrupt flags can therefore not
 * always be saved to the stack.  Instead the critical section nesting level
 * is stored in a variable, which is then saved as part of the stack context. 
 */
...
{
    Set_system_register (AVR32_COMPARE,
...
static void prvSetupTimerInterrupt (void)
{
    ...
    tc_waveform_opt_t waveform_opt = {
        ...
        .acpa = TC_EVT_EFFECT_NOOP, /* RA compare effect on TIOA: toggle
                                     * (other possibilities are none, set and 
                                     * clear). */
...
#if( configTICK_USE_TC==1 )
    {
        ...
        /*
         * Set the compare triggers. Remember TC counter is 16-bits, so
         * counting second is not possible! That's why we configure it to
         * count ms. Set Rc to the right value with rounding.
         */
        tc_write_rc (tc, configTICK_TC_CHANNEL,
                     (configCPU_CLOCK_HZ +
                      4 * configTICK_RATE_HZ) / (8 * configTICK_RATE_HZ));
...
#else
    {
        INTC_register_interrupt (&vTick, AVR32_CORE_COMPARE_IRQ,
                                 AVR32_INTC_INT0);
