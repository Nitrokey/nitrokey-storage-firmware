...
static xList xDelayedCoRoutineList2;    /* < Delayed co-routines (two lists
                                         * are used - one for delays that
                                         * have overflowed the current tick
                                         * count. */
static xList *pxDelayedCoRoutineList;   /* < Points to the delayed co-routine 
...
static xList *pxOverflowDelayedCoRoutineList;   /* < Points to the delayed
                                                 ...
                                                 * tick count. */
static xList xPendingReadyCoRoutineList;    /* < Holds co-routines that have
                                             * been readied by an external
                                             * event.  They cannot be added
                                             * directly to the ready lists as 
                                             * the ready lists cannot be
...
static portTickType xCoRoutineTickCount = 0, xLastTickCount =
    0, xPassedTicks = 0;
                                       ...
                                       unsigned portBASE_TYPE uxIndex)
{
    ...
    if (pxCoRoutine)
    {
        ...
        /*
         * Fill out the co-routine control block from the function
         * parameters. 
         */
        ...
        /*
         * Set the co-routine control block as a link back from the
         * xListItem. This is so we can get back to the containing CRCB from
        ...
        /*
         * Now the co-routine has been initialised it can be added to the
         * ready list at the correct priority. 
         */
...
portTickType xTimeToWake;
    ...
    if (xTimeToWake < xCoRoutineTickCount)
    {
         ...
         */
        vListInsert ((xList *) pxOverflowDelayedCoRoutineList,
                     (xListItem *) & (pxCurrentCoRoutine->xGenericListItem));
    }
    else
    {
        /*
         * The wake time has not overflowed, so we can use the current block
         * list. 
         */
        vListInsert ((xList *) pxDelayedCoRoutineList,
                     (xListItem *) & (pxCurrentCoRoutine->xGenericListItem));
    }
    ...
    {
        /*
         * Also add the co-routine to an event list.  If this is done then
...
{
    /*
     * Are there any co-routines waiting to get moved to the ready list?
     * These are co-routines that have been readied by an ISR.  The ISR
     * cannot access the ready lists itself. 
     */
    ...
    corCRCB *pxUnblockedCRCB;
        ...
        portDISABLE_INTERRUPTS ();
        {
            pxUnblockedCRCB =
                (corCRCB *)
                listGET_OWNER_OF_HEAD_ENTRY ((&xPendingReadyCoRoutineList));
...
static void prvCheckDelayedList (void)
{
    ...
    xList *pxTemp;
             ...
             * If there are any items in pxDelayedCoRoutineList here then
             * there is an error! 
             */
        ...
        while ((pxCRCB =
                (corCRCB *)
                listGET_OWNER_OF_HEAD_ENTRY (pxDelayedCoRoutineList)) != NULL)
        {
            ...
            portDISABLE_INTERRUPTS ();
            {
                 ...
                 * parameter will have been set to NULL so the following
                 * lines are also valid. 
                 */
...
signed portBASE_TYPE xCoRoutineRemoveFromEventList (const xList * pxEventList)
{
    ...
    vListRemove (&(pxUnblockedCRCB->xEventListItem));
    vListInsertEnd ((xList *) & (xPendingReadyCoRoutineList),
