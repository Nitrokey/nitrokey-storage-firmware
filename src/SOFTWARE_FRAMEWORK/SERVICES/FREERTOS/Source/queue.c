...
{
    signed char *pcHead;    /* < Points to the beginning of the queue storage 
                             * area. */
    ...
    signed char *pcWriteTo; /* < Points to the free next place in the storage 
                             * area. */
    ...
    xList xTasksWaitingToSend;  /* < List of tasks that are blocked waiting
                                 ...
                                 * priority order. */
    xList xTasksWaitingToReceive;   /* < List of tasks that are blocked
                                     * waiting to read from this queue.
                                     * Stored in priority order. */

    volatile unsigned portBASE_TYPE uxMessagesWaiting;  /* < The number of
                                                         * items currently in 
                                                         * the queue. */
    unsigned portBASE_TYPE uxLength;    /* < The length of the queue defined
                                         * as the number of items it will
    ...
    signed portBASE_TYPE xRxLock;   /* < Stores the number of items received
                                     * from the queue (removed from the
                                     * queue) while the queue was locked.
                                     * Set to queueUNLOCKED when the queue is 
                                     * not locked. */
    signed portBASE_TYPE xTxLock;   /* < Stores the number of items
                                     * transmitted to the queue (added to the 
                                     * queue) while the queue was locked.
                                     * Set to queueUNLOCKED when the queue is 
                                     * not locked. */
 ...
 */
    ...
    /*
     * The queue registry is simply an array of xQueueRegistryItem
     * structures. The pcQueueName member of a structure being NULL is
     * indicative of the array position being vacant. 
     */
...
static void prvCopyDataToQueue (xQUEUE * pxQueue, const void *pvItemToQueue,
                                portBASE_TYPE xPosition) PRIVILEGED_FUNCTION;
                           ...
                           unsigned portBASE_TYPE uxItemSize)
{
    ...
    if (uxQueueLength > (unsigned portBASE_TYPE) 0)
    {
        ...
        if (pxNewQueue != NULL)
        {
             ...
             * byte longer than asked for to make wrap checking
             * easier/faster. 
             */
            xQueueSizeInBytes =
    ...
    /*
     * Will only reach here if we could not allocate enough memory or no
     * memory was required. 
     */
    return NULL;
}
...
xQUEUE *pxNewQueue;
    ...
    if (pxNewQueue != NULL)
    {
        ...
        /*
         * Queues used as a mutex no data is actually copied into or out of
         * the queue. 
         */
...
portBASE_TYPE xReturn;
     ...
     * change outside of this task.  If this task does not hold the mutex
     * then pxMutexHolder can never coincidentally equal the tasks handle,
     * and as this is the only condition we are interested in it does not
     * matter if pxMutexHolder is accessed simultaneously by another task.
     * Therefore no mutual exclusion is required to test the pxMutexHolder
     * variable. 
     */
    if (pxMutex->pxMutexHolder == xTaskGetCurrentTaskHandle ())
    {
         ...
         * there can only be one, no mutual exclusion is required to modify
         * the uxRecursiveCallCount member. 
         */
...
portBASE_TYPE xReturn;
    ...
    else
    {
        ...
        /*
         * pdPASS will only be returned if we successfully obtained the
         * mutex, we may have blocked to reach here. 
         */
        if (xReturn == pdPASS)
        {
            (pxMutex->uxRecursiveCallCount)++;
    ...
    for (;;)
    {
            ...
            if (pxQueue->uxMessagesWaiting < pxQueue->uxLength)
            {
                    ...
                    pdFALSE)
                {
                        ...
                        (&(pxQueue->xTasksWaitingToReceive)) == pdTRUE)
                    {
                         ...
                         * from within the critical section - the kernel
                         * takes care of that. 
                         */
        ...
        if (xTaskCheckForTimeOut (&xTimeOut, &xTicksToWait) == pdFALSE)
        {
            if (prvIsQueueFull (pxQueue))
            {
                 ...
                 * ready list into the ready list - so it is feasible that
                 * this task is already in a ready list before it yields - in 
                 * which case the yield will not cause a context switch
                 * unless there is also a higher priority task in the pending 
                 * ready list. 
                 */
                if (!xTaskResumeAll ())
                {
        ...
        taskENTER_CRITICAL ();
        {
            if (pxQueue->uxMessagesWaiting > (unsigned portBASE_TYPE) 0)
            {
                ...
                if (xJustPeeking == pdFALSE)
                {
...
#if ( configUSE_MUTEXES == 1 )
                    {
                        if (pxQueue->uxQueueType == queueQUEUE_IS_MUTEX)
                        {
                             ...
                             * priority inheritance should it become
                             * necessary. 
                             */
                ...
                else
                {
                    ...
                    {
                        /*
                         * Tasks that are removed from the event list will
                         * get added to the pending ready list as the
                         * scheduler is still suspended. 
                         */
                        if (xTaskRemoveFromEventList
                            (&(pxQueue->xTasksWaitingToReceive)) != pdFALSE)
                        {
                            /*
                             * The task waiting has a higher priority than
                             * this task. 
                             */
                            portYIELD_WITHIN_API ();
                        }
                                               ...
                                               portBASE_TYPE xCopyPosition)
{
    ...
    /*
     * Similar to xQueueGenericSend, except we don't block if there is no
     * room in the queue.  Also we don't directly wake a task that was
     * blocked on a queue read, instead we return a flag to say whether a
     * context switch is required or not (i.e. has a task with a higher
     * priority than us been woken by this post). 
     */
    uxSavedInterruptStatus = portSET_INTERRUPT_MASK_FROM_ISR ();
    {
        if (pxQueue->uxMessagesWaiting < pxQueue->uxLength)
        {
            ...
            {
                /*
                 * Increment the lock count so the task that unlocks the
                 * queue knows that data was posted while it was locked. 
                 */
                ++(pxQueue->xTxLock);
            }
                                           ...
                                           signed portBASE_TYPE * pxTaskWoken)
{
    ...
    uxSavedInterruptStatus = portSET_INTERRUPT_MASK_FROM_ISR ();
    {
        ...
        if (pxQueue->uxMessagesWaiting > (unsigned portBASE_TYPE) 0)
        {
             ...
             * queue will know that an ISR has removed data while the queue
             * was locked. 
             */
            if (pxQueue->xRxLock == queueUNLOCKED)
            {
            ...
            else
            {
                /*
                 * Increment the lock count so the task that unlocks the
                 * queue knows that data was removed while it was locked. 
                 */
                ++(pxQueue->xRxLock);
            }
...
static void prvUnlockQueue (xQueueHandle pxQueue)
{
    ...
    taskENTER_CRITICAL ();
    {
        ...
        while (pxQueue->xTxLock > queueLOCKED_UNMODIFIED)
        {
            ...
            {
                /*
                 * Tasks that are removed from the event list will get added
                 * to the pending ready list as the scheduler is still
                 * suspended. 
                 */
                if (xTaskRemoveFromEventList
                    (&(pxQueue->xTasksWaitingToReceive)) != pdFALSE)
                                   ...
                                   portTickType xTicksToWait)
{
     ...
     * between the check to see if the queue is full and blocking on the
     * queue. 
     */
    portDISABLE_INTERRUPTS ();
    {
        if (prvIsQueueFull (pxQueue))
        {
            ...
            {
                /*
                 * As this is called from a coroutine we cannot block
                 * directly, but return indicating that we need to block. 
                 */
    ...
    portDISABLE_INTERRUPTS ();
    {
        if (pxQueue->uxMessagesWaiting < pxQueue->uxLength)
        {
            ...
            if (!listLIST_IS_EMPTY (&(pxQueue->xTasksWaitingToReceive)))
            {
                 ...
                 * Instead the same pending ready list mechanism is used as
                 * if the event were caused from within an interrupt. 
                 */
                                      ...
                                      portTickType xTicksToWait)
{
    ...
    /*
     * If the queue is already empty we may have to block.  A critical
     * section is required to prevent an interrupt adding something to the
     * queue between the check to see if the queue is empty and blocking on
     * the queue. 
     */
    portDISABLE_INTERRUPTS ();
    {
        ...
        {
            /*
             * There are no messages in the queue, do we want to block or
             * just leave with nothing? 
             */
            if (xTicksToWait > (portTickType) 0)
            {
                /*
                 * As this is a co-routine we cannot block directly, but
