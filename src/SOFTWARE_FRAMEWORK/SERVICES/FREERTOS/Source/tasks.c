...
typedef struct tskTaskControlBlock
{
    ...
    unsigned portBASE_TYPE uxBasePriority;  /* < The priority last assigned
                                             * to the task - used by the
    ...
    unsigned long long ullRunTimeCounter;   /* < Used for calculating how
                                             * much CPU time each task is
                                             * utilising. */
...
PRIVILEGED_DATA static xList xDelayedTaskList2; /* < Delayed tasks (two lists 
                                                 ...
                                                 * current tick count. */
PRIVILEGED_DATA static xList *volatile pxDelayedTaskList;   /* < Points to
                                                             * the delayed
                                                             * task list
                                                             * currently
                                                             * being used. */
...
PRIVILEGED_DATA static xList xPendingReadyList; /* < Tasks that have been
                                                 ...
                                                 * ready queue when the
                                                 * scheduler is resumed. */

#if ( INCLUDE_vTaskDelete == 1 )

PRIVILEGED_DATA static volatile xList xTasksWaitingTermination; /* < Tasks
                                                                 * that have
                                                                 * been
                                                                 * deleted -
                                                                 * but the
...
PRIVILEGED_DATA static unsigned long long ullTaskSwitchedInTime = 0UL;  /* <
                                                                         * Holds 
                                                                         * the 
                                                                         * value 
                                                                         * of 
                                         ...
                                         const xMemoryRegion * const xRegions)
{
    ...
    portSTACK_TYPE *pxTopOfStack;
         ...
         * running, but had been interrupted by the scheduler.  The return
         * address is set to the start of the task function. Once the stack
        ...
        {
            pxNewTCB->pxTopOfStack =
                pxPortInitialiseStack (pxTopOfStack, pxTaskCode,
                                       pvParameters);
            (void) xRunPrivileged;
        }
#endif
        ...
        portENTER_CRITICAL ();
        {
            ...
            {
                /*
                 * As this is the first task it must also be the current
                 * task. 
                 */
            ...
            {
                /*
                 * If the scheduler is not already running, make this task
                 * the current task if it is the highest priority task to be
                 * created so far. 
                 */
             ...
             * Use the priority in pxNewTCB as this has been capped to a
             * valid value. 
             */
...
tskTCB *pxTCB;
    ...
    portENTER_CRITICAL ();
    {
         ...
         * check the termination list and free up any memory allocated by the
                      ...
                      portTickType xTimeIncrement)
{
    ...
    vTaskSuspendAll ();
    {
        ...
        if (xTickCount < *pxPreviousWakeTime)
        {
             ...
             * delay is if the wake time has also overflowed, and the wake
             * time is greater than the tick time.  When this is the case it
             * is as if neither time had overflowed. 
             */
        ...
        {
            /*
             * The tick time has not overflowed.  In this case we will delay
             * if either the wake time has overflowed, and/or the tick time
             * is less than the wake time. 
             */
            if ((xTimeToWake < *pxPreviousWakeTime)
                || (xTimeToWake > xTickCount))
            {
...
signed portBASE_TYPE xAlreadyYielded = pdFALSE;
    ...
    if (xTicksToDelay > (portTickType) 0)
    {
        vTaskSuspendAll ();
        {
             ...
             * from the blocked list until the scheduler is resumed.
             * 
            ...
            /*
             * Calculate the time to wake - this may overflow but this is not 
             * a problem. 
             */
        ...
        if (uxCurrentPriority != uxNewPriority)
        {
            ...
            if (uxNewPriority > uxCurrentPriority)
            {
                if (pxTask != NULL)
                {
                     ...
                     * were raising the priority of the currently running
                     * task there would be no need to switch as it must have
                     * already been the highest priority task. 
                     */
            ...
            {
                /*
                 * Setting our own priority down means there may now be
                 * another task of higher priority that is ready to execute. 
                 */
                xYieldRequired = pdTRUE;
            }
...
const tskTCB *const pxTCB = (tskTCB *) xTask;
    ...
    /*
     * Is the task we are attempting to resume actually in the suspended
     * list? 
     */
    if (listIS_CONTAINED_WITHIN
        (&xSuspendedTaskList, &(pxTCB->xGenericListItem)) != pdFALSE)
    {
            ...
            (&xPendingReadyList, &(pxTCB->xEventListItem)) != pdTRUE)
        {
             ...
             * state? It is possible to be in the suspended list because it
             * is blocked on a task with no timeout specified. 
             */
...
tskTCB *pxTCB;
    ...
    /*
     * Remove the task from whichever list it is currently in, and place it
     * in the ready list. 
     */
...
void vTaskStartScheduler (void)
{
    ...
    if (xReturn == pdPASS)
    {
         ...
         * generate the run time counter time base. 
         */
...
signed portBASE_TYPE xTaskResumeAll (void)
{
    ...
    portBASE_TYPE xYieldRequired = pdFALSE;
                       ...
                       NULL)
                {
                    ...
                    /*
                     * If we have moved a task that has a priority higher
                     * than the current task then we should yield. 
                     */
                    if (pxTCB->uxPriority >= pxCurrentTCB->uxPriority)
                    {
                        xYieldRequired = pdTRUE;
                ...
                if (uxMissedTicks > (unsigned portBASE_TYPE) 0)
                {
                     ...
                     * yield to ensure the highest priority task that is
                     * ready to run is the task actually running. 
                     */
...
void vTaskSwitchContext (void)
{
    ...
    unsigned long long ullTempCounter = portGET_RUN_TIME_COUNTER_VALUE ();
         ...
         * overflows.  Generally this will be about 1 hour assuming a 1uS
         * timer increment. 
         */
        pxCurrentTCB->ullRunTimeCounter +=
            (ullTempCounter - ullTaskSwitchedInTime);
        pxCurrentTCB->ullRunTimeCounterMinute +=
            (ullTempCounter - ullTaskSwitchedInTime);
                            ...
                            portTickType xTicksToWait)
{
     ...
     * to the blocked list as the same list item is used for both lists.  We
...
#if ( INCLUDE_vTaskSuspend == 1 )
    {
        if (xTicksToWait == portMAX_DELAY)
        {
             ...
             * task list to ensure we are not woken by a timing event.  We
                                               ...
                                               const pxEventList)
{
     ...
     * instead.  This means we can always expect exclusive access to the
     * event list here. 
     */
                                    ...
                                    portTickType * const pxTicksToWait)
{
    ...
    portENTER_CRITICAL ();
    {
        ...
        else    /* We are not blocking indefinitely, perform the checks
                 * below. */
#endif
        ...
        else if (((portTickType) xTickCount -
                  (portTickType) pxTimeOut->xTimeOnEntering) <
...
static portTASK_FUNCTION (prvIdleTask, pvParameters)
{
    ...
    for (;;)
    {
        ...
        {
            /*
             * If we are not using preemption we keep forcing a task switch
             * to see if any other task has become available.  If we are
             * using preemption we don't need to do this as any task becoming 
...
#if ( ( configUSE_PREEMPTION == 1 ) && ( configIDLE_SHOULD_YIELD == 1 ) )
        {
             ...
             * from the list, and an occasional incorrect value will not
             * matter.  If the ready list at the idle priority contains more
             * than one task then a task other than the idle task is ready to 
             * execute. 
             */
...
extern void vApplicationIdleHook (void);
             ...
             * This allows the application designer to add background
             * functionality without the overhead of a separate task. NOTE:
             * vApplicationIdleHook() MUST NOT, UNDER ANY CIRCUMSTANCES, CALL 
...
}   /* lint !e715 pvParameters is not accessed but all task functions require 
     * the same prototype. */
                                       ...
                                       unsigned short usStackDepth)
{
    ...
    /*
     * Set the pxTCB as a link back from the xListItem.  This is so we can
    ...
    do
    {
        ...
        if (ullTotalRunTime > 0UL)
        {
            ...
            else
            {
                 ...
                 */
                ulStatsAsPercentage =
                    (100UL * pxNextTCB->ullRunTimeCounter) / ullTotalRunTime;

                if (ulStatsAsPercentage > 0UL)
                {
                    sprintf (pcStatsString, (char *) "%s\t\t%u\t\t%u%%\r\n",
                             pxNextTCB->pcTaskName,
                             (unsigned int) pxNextTCB->ullRunTimeCounter,
...
xTaskHandle xTaskGetCurrentTaskHandle (void)
{
     ...
     * interrupt and the current TCB will always be the same for any
