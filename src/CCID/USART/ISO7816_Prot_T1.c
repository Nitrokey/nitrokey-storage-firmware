...
#define ISO7816_MAX_SENDBUFFER			300 // NAD 1 + PCB 1 + LEN 2 + APDU
                                            // 254 + EDC 2 = 260 (+2 for
                                            // secure)
#define ISO7816_MAX_RECEIVEBUFFER		300 // NAD 1 + PCB 1 + LEN 2 + APDU
                                            // 254 + EDC 2 = 260 (+2 for
                                            // secure)

#define ISO7816_MAX_SEND_HEADER     6   // NAD 1 + PCB 1 + LEN 2 = 4 (+2 for
                                        // secure)
...
unsigned char ISO7816_T1_BlockSize = 32;    // 32; // Standard block size (=
                                            // IFSD)
...
int ISO7816_T1_CheckReceiveBlock (unsigned int nResponseLen)
{
    ...
    switch (nBlockKind)
    {
        case ISO7816_T1_BLOCK_KIND_I_BLOCK:
            if ((ISO7816_T1_ReceiveBuffer[ISO7816_T1_PCB_ADDR] &
                 ISO7816_T1_PCB_SEND_NR_BIT_I_BLOCK) !=
                (ISO7816_T1_Get_I_BlockNr () *
                 ISO7816_T1_PCB_SEND_NR_BIT_I_BLOCK))
            {
                if ((ISO7816_T1_SendBuffer[ISO7816_T1_PCB_ADDR] & ISO7816_T1_PCB_CHAINING_BIT_I_BLOCK) != 0)    // Todo 
                                                                                                                // check: 
                                                                                                                // When 
                                                                                                                // no 
                                                                                                                // chaining 
...
unsigned int ISO7816_T1_BuildSendHeader (unsigned int nRestBlocklen)
{
    ...
    // PCB
    ISO7816_T1_DataBlock.cHeaderBuffer[ISO7816_T1_DataBlock.cHeaderSize] =
    ...
    {
        ISO7816_T1_DataBlock.cHeaderBuffer[ISO7816_T1_DataBlock.
                                           cHeaderSize] |=
            ISO7816_T1_PCB_CHAINING_BIT_I_BLOCK;
    ...
    // LEN
    ISO7816_T1_DataBlock.cHeaderBuffer[ISO7816_T1_DataBlock.cHeaderSize] =
        nSendBlockLen + ISO7816_T1_PROTOCOL_OVERHEAD;
    ISO7816_T1_DataBlock.cHeaderSize++;
...
int ISO7816_T1_Send_Block (void)
{
    ...
    // Send T1 header
    nRet =
        ISO7816_SendString (ISO7816_T1_DataBlock.cHeaderSize,
                            ISO7816_T1_DataBlock.cHeaderBuffer);
    if (USART_SUCCESS != nRet)
    {
        return (FALSE);
    }
    ...
    // Send data from apdu
    nRet =
        ISO7816_SendString (ISO7816_T1_DataBlock.nDataSize,
                            ISO7816_T1_DataBlock.cDataBuffer);
    if (USART_SUCCESS != nRet)
    {
        return (FALSE);
    }
    ...
    // Send EDC
    nRet =
        ISO7816_SendString (ISO7816_T1_DataBlock.cEDC_Size,
                            ISO7816_T1_DataBlock.cEDC_Buffer);
    if (USART_SUCCESS != nRet)
    {
        return (FALSE);
    }
...
void ISO7816_T1_Send_APDU_String (unsigned int nAPDULen,
                                  unsigned char *cAPDUData,
                                  ...
                                  unsigned char *cReceiveData)
{
    ...
    *nReceiveBytes = 0;
    ...
    while (nAPDULen > nSendPointer)
    {
        ...
        // Receive block
        nRet = ISO7816_T1_ReceiveBlock (&nResponseLen); // to do: use
                                                        // "cReceiveData"
                                                        // buffer
        if (FALSE == nRet)
        {
        ...
        {
            if (ISO7816_T1_RECEIVE_R_BLOCK !=
                ISO7816_T1_CheckReceiveBlock (nResponseLen))
            {
...
int ISO7816_T1_SendBlock (unsigned int nCount, unsigned char *cData,
                          unsigned int nChainingFlag)
{
    ...
    {
        ISO7816_T1_SendBuffer[nPointer] |=
            ISO7816_T1_PCB_CHAINING_BIT_I_BLOCK;
    }
    nPointer++;
    ...
    *nReceiveBytes = 0;
    ...
    while (nSendBytes > nSendPointer)
    {
        ...
        if (ISO7816_T1_SEND_I_BLOCK == nSendFlag)
        {
            ...
            // Send block to smartcard via T1
            nRet =
                ISO7816_T1_SendBlock (nBlockSize, &cSendData[nSendPointer],
                                      nChainingFlag);
            if (FALSE == nRet)
            {
                return (FALSE);
        ...
        switch (nRet)
        {
            case ISO7816_T1_RECEIVE_I_BLOCK:
                memcpy (&cReceiveData[nReceivePointer],
                        &ISO7816_T1_ReceiveBuffer[3], nResponseLen - 4);
                nReceivePointer += nResponseLen - 4;
                nSendPointer += nBlockSize;
                nSendFlag = ISO7816_T1_SEND_I_BLOCK;
                ISO7816_T1_Add_I_BlockNr ();    // Switch block nr for next
                                                // i-block
                break;
...
int ISO7816_T1_ReceiveBlock (unsigned int *nCount)
{
    ...
    if (ISO7816_T1_ReceiveBuffer[nResponseLen - 1] !=
        ISO7816_XOR_String (nResponseLen - 1, ISO7816_T1_ReceiveBuffer))
    {
        return (FALSE);
    }
    ...
    // check send counter
    /*
     * Check Full PCB if ((ISO7816_T1_SendBuffer[ISO7816_T1_PCB_ADDR] &
     * ISO7816_T1_PCB_SEND_NR_BIT_I_BLOCK) != (ISO7816_T1_GetLastSendNr () *
                              ...
                              unsigned char *cReceiveBuffer)
{
    ...
    while (FALSE == nEndFlag)
    {
        ...
        memcpy (&cReceiveBuffer[nReceivePos], &ISO7816_T1_ReceiveBuffer[3],
                nResponseLen - 4);
...
int nRet;
    ...
    // Send APDU via T1
    nRet =
        ISO7816_T1_SendString (tSC->nSendDataLen, tSC->cSendData,
                               &tSC->nReceiveDataLen, tSC->cReceiveData);
    if (TRUE != nRet)
    {
        return (FALSE);
    }
                          ...
                          signed long nInputLength)
{
    ...
    *nLength = nInputLength;
    ...
    nCount++;
    /*
     * if (ISO7816_USART_TIMEOUT == nRet) { CI_LocalPrintf ("%7d :
     * ISO7816_T1_DirectXfr - Timeout\n",xTaskGetTickCount()); } 
     */
