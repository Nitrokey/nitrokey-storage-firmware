...
u8 Text[20];
    ...
    while (0 != USB_CCID_GetLockCounter ())
    {
        ...
        LockActive_u32 = TRUE;
        CI_TickLocalPrintf
            ("ISO7816 - *** WAIT for unlock CCID counter - %3d msec***\r\n",
...
int nRet;
    ...
    usart_reset_tx (ISO7816_USART); // reset the tx port to free the port for 
                                    // receiving - Chip bug
...
unsigned int FiDiRatio;
    ...
    /*
     */
    cd = (CCID_TARGET_PBACLK_FREQ_HZ + ISO7816_USART_ISO7816.iso7816_hz / 2) / ISO7816_USART_ISO7816.iso7816_hz;    // = 
                                                                                                                    // frequency 
                                                                                                                    // divisor
...
int nRet;
    ...
    memcpy (&tSC->cSendData[tSC->nSendDataLen], tSC->tAPDU.cData,
            (tSC->tAPDU.nLc & 0xFF));
...
int nRet;
    ...
    if (256 > tSC->tAPDU.nLe)
    {
        tSC->cSendData[tSC->nSendDataLen] =
            (unsigned char) (tSC->tAPDU.nLe & 0xFF);
        tSC->nSendDataLen += 1;
    }
    else
    {
        tSC->cSendData[tSC->nSendDataLen] =
            (unsigned char) (tSC->tAPDU.nLe >> 8);
        tSC->nSendDataLen += 1;
        tSC->cSendData[tSC->nSendDataLen] =
            (unsigned char) (tSC->tAPDU.nLe & 0xFF);
        tSC->nSendDataLen += 1;
...
void ISO7816_APDU_Test (void)
{
     ...
     * 00 01 24 01 gpg: DBG: response: sw=9000 datalen=0 gpg: DBG: dump: 2D 
     */
