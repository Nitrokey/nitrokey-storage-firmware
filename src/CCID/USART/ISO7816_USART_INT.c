...
#define EXAMPLE_INT_TARGET_PBACLK_FREQ_HZ FOSC0 // PBA clock target
                                                // frequency, in Hz
#else
...
*******************************************************************************/
static ISO7816_USART_INT_Status ISO7816_UsartRxBufferStatus_e =
    ISO7816_USART_INT_STATUS_OK;
...
u16 SaveBufferPointer_u16;
    ...
    {
        // Something received ?
        if (ISO7816_UsartRxBufferPointerEnd_u16 !=
            ISO7816_UsartRxBufferPointerStart_u16)
        {
            // Yes
            *nChar_pu32 =
                ISO7816_UsartRxBuffer_u8
                [ISO7816_UsartRxBufferPointerStart_u16];
...
void ISO7816_Usart_ISR_Init (void)
{
    ...
    // Set ISR
    INTC_register_interrupt (&ISO7816_Usart_ISR, EXAMPLE_INT_USART_IRQ,
                             AVR32_INTC_INT0);
...
void ISO7816_Usart_ISR_Test (void)
{
    ...
    if (1 == EXAMPLE_INT_USART->IMR.txempty)
    {
        ...
        if (1 == EXAMPLE_INT_USART->CSR.txempty)
        {
            ...
            /*
             * // Disable tx empty interrupt Disable_global_interrupt();
             * EXAMPLE_INT_USART->IDR.txempty = 1; Enable_global_interrupt();
             * while (1 == EXAMPLE_INT_USART->IMR.txempty) ; 
             */
            /*
             * // clearing interrupt request x = EXAMPLE_INT_USART->cr;
             * EXAMPLE_INT_USART->cr = x; x = EXAMPLE_INT_USART->cr; 
             */
        }
    }
    else
...
void USART_Int_Test (void)
{
    ...
    // sleep
    // mode: we just need to be sure that the USART module will be still be
    // active
    // in the chosen sleep mode. The sleep mode to use is the FROZEN sleep
    // mode:
    // in this mode the PB clocks are still active (so the USART module which 
    // is
    // on the Peripheral Bus will still be active while the CPU and HSB will
    // be
    // stopped).
    // --
    // Modules communicating with external circuits should normally be
    // disabled
    // before entering a sleep mode that will stop the module operation: this 
    // is not
    // the case for the FROZEN sleep mode.
    // --
    // When the USART interrupt occurs, this will wake the CPU up which will
    // then
    // execute the interrupt handler code then come back to the while(1) loop 
    // below
    ...
    while (1)
    {
        ...
        EXAMPLE_INT_USART->IER.txempty = 1;
         /**/
            ...
            // incomplete, the CPU
            // should perform a read operation from any register on the PB
            // bus before
