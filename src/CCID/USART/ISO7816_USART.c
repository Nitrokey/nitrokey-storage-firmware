...
int Init_ISO7816_Usart (void)
{
    ...
    nRet =
        gpio_enable_module (ISO7816_USART_GPIO_MAP,
                            sizeof (ISO7816_USART_GPIO_MAP) /
                            sizeof (ISO7816_USART_GPIO_MAP[0]));

    // Init ISO 7816
    nRet =
        usart_init_iso7816 (ISO7816_USART, &ISO7816_USART_ISO7816, nProtocol,
                            CCID_TARGET_PBACLK_FREQ_HZ);
...
int usart_set_fidi_ratio (volatile avr32_usart_t * usart,
                          unsigned long nFidiRatio)
{
...
/*
 * inline int ISO7816_Is_RX_Rready (void) { // return
 * (usart_rx_ready(ISO7816_USART)); } 
 */
/*******************************************************************************
...
int ISO7816_ReadCharNoISR (int *nChar, int nTimeOutMs)
{
    ...
    nDelayCounter = (nTimeOutMs * ISO7816_CHAR_DELAY_COUNTER_1MS) + 1;  // +
                                                                        // 1
                                                                        // =
                                                                        // for 
                                                                        // no 
...
int ISO7816_SendString (int nCount, unsigned char *cString)
{
    ...
    for (i = 0; i < nCount; i++)
    {
        ...
        nTimeOutMs = ISO7816_CHAR_DELAY_TIME_MS;    // next char > short
                                                    // delay
    }
...
void ISO7816_ResetSC_test (void)
{
    ...
    // usart_reset_tx (ISO7816_USART); // reset the tx port to free the port
    // for receiving
