...
void Print_T1_Header_full (unsigned char *acBlockData)
{
    CI_LocalPrintf
        ("\r\n------------------------------------\r\nNAD-Block: ");
    CI_LocalPrintf ("DAD (Ziel)   %d  Vqq %d -", acBlockData[0] >> 7,
                    (acBlockData[0] >> 4) & 0x7);
...
void Print_USB_to_CCID_T1_Data_I_Block (int nLenght,
                                        unsigned char *acStartBlockData)
{
    /*
     * CI_LocalPrintf (" CLA INS P1 P2 Lc \r\n"); CI_LocalPrintf (" %02x %02x 
     * %02x
     * %0n",acStartBata[0ata[0],acBlockDaockDatacStaacStartBlockDatakData[2],acStartBlockData[3]); 
     */

    // Select file
    if ((0xA4 == acStartBlockData[1]) && (0x04 == acStartBlockData[2])
        && (0x00 == acStartBlockData[3]))
    {
    ...
    // GET RESPONSE
    if ((0xC0 == acStartBlockData[1]) && (0x00 == acStartBlockData[2])
        && (0x00 == acStartBlockData[3]))
    {
    ...
    // GET CHALLENGE
    if ((0x84 == acStartBlockData[1]) && (0x00 == acStartBlockData[2])
        && (0x00 == acStartBlockData[3]))
    {
...
void Print_T1_I_Block_Data (int nLenght, unsigned char *acBlockData)
{
    ...
    {
        CI_LocalPrintf ("          Folgedatenblock - %2d Bytes = ",
                        acBlockData[2]);
    ...
    else
    {
        ...
        CI_LocalPrintf ("= ");
        nStatusByte =
            acBlockData[nLenght - 3] * 0x100 + acBlockData[nLenght - 2];
        switch (nStatusByte)
        {
                /*
                 * 61 xx Command correct, xx bytes available in response
                 * (normally used under T=0 or for commands under any
                 * protocol with long response data that cannot be
                 * transmitted in one response) 
                 */
            case 0x6285:
            ...
            case 0x6982:
                ...
                /*
                 * PW wrong PW not checked (command not allowed) Secure
                 * messaging incorrect (checksum and/or cryptogram) 
                 */
                break;
            case 0x6983:
...
void LogEnd_T1_Block (int nLenght, unsigned char acEndBlockData[])
{
    ...
    CI_LocalPrintf ("%7d : Send T1 block - Comand len %3d - ",
                    nUSB_To_CCID_StartTime, nStartT1BlockLenght);
    CI_LocalPrintf (" Answer len %3d - %5d ms\n", nLenght,
                    nUSB_To_CCID_EndTime - nUSB_To_CCID_StartTime);
    ...
    if (3 == CCID_T1_DebugLevel)
    {
        ...
        switch (acStartT1Block[1] >> 6)
        {
            ...
            case 1:
                CI_LocalPrintf ("          I-Block\r\n");
                Print_USB_to_CCID_T1_Data_I_Block (nStartT1BlockLenght - 3,
                                                   (unsigned char *)
                                                   &acStartT1Block[3]);
                break;
            case 2:    // R-Block
