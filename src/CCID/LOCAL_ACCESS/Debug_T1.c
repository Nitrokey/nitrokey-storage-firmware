...
{
  CI_LocalPrintf ("\r\n------------------------------------\r\nNAD-Block: ");
  CI_LocalPrintf ("DAD (Ziel)   %d  Vqq %d -"
      , acBlockData[0] >> 7
      ,(acBlockData[0] >> 4) & 0x7);
...
//  CI_LocalPrintf ("          Length %d\r\n",acBlockData[2]);
  ...
  /*
    61
    xx
    Command correct, xx bytes available in response (normally used under T=0 or for commands under any protocol with long response data that cannot be transmitted in one response)
  */
      case 0x6285 :
