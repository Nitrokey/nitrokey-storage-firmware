...
int nRet;
    ...
    /*
     * SELECT FILE - so übernehmen 00 00 0B gpg: DBG: send apdu: c=00 i=A4
     * p1=04 p2=00 lc=6 le=-1 em=0 gpg: DBG: PCSC_data: 00 A4 04 00 06 D2 76
     * 00 01 24 01 gpg: DBG: response: sw=9000 deadline=0 gpg: DBG: dump: 2D 
     */
...
int LA_OpenPGP_V20_GetData (typeAPDU * tSC, unsigned char cP1,
                            unsigned char cP2)
{
    int nRet;
...
int LA_OpenPGP_V20_ResetRetryCounter (typeAPDU * tSC,
                                      unsigned char cPasswordLen,
                                      unsigned char *pcPassword)
{
    int nRet;
    ...
    if (ISO7816_MAX_APDU_DATA + ISO7816_APDU_SEND_HEADER_LEN +
        ISO7816_APDU_OFERHEAD <= cPasswordLen)
    {
        return (FALSE);
    }
...
int LA_OpenPGP_V20_Verify (typeAPDU * tSC, unsigned char cPW,
                           unsigned char cPasswordLen,
                           unsigned char *pcPassword)
{
    int nRet;
...
int LA_OpenPGP_V20_GetChallenge (typeAPDU * tSC, int nReceiveLength,
...
int LA_OpenPGP_V20_GetPublicKey (typeAPDU * tSC, int nKind,
                                 int nReceiveLength,
                                 unsigned char **cReceiveData)
{
    int nRet;
    ...
    /*
     * Not necessary n = tSC->nReceiveDataLen; if (n < nReceiveLength) { n =
     * nReceiveLength; } 
     */
...
int LA_OpenPGP_V20_ComputeSignature (typeAPDU * tSC, int nSendLength,
...
int LA_OpenPGP_V20_Decipher (typeAPDU * tSC, int nSendLength,
                             unsigned char *cSendData, int nReceiveLength,
                             unsigned char *cReceiveData)
{
    int nRet;
...
int LA_OpenPGP_V20_Put_AES_key (typeAPDU * tSC, unsigned char cKeyLen,
                                unsigned char *pcAES_Key)
{
    int nRet;
...
int LA_OpenPGP_V20_AES_Enc (typeAPDU * tSC, int nSendLength,
                            unsigned char *cSendData, int nReceiveLength,
                            unsigned char *cReceiveData)
{
    int nRet;
...
int LA_OpenPGP_V20_AES_Dec_SUB (typeAPDU * tSC, int nSendLength,
                                unsigned char *cSendData, int nReceiveLength,
                                unsigned char *cReceiveData)
{
    int nRet;
    ...
    /*
     * Don't restart because the password access is lost if (FALSE ==
     * LA_SC_StartSmartcard ()) { return (FALSE); } 
     */
    // Command
    tSC->tAPDU.cCLA = 0x00;
    tSC->tAPDU.cINS = 0x2A;
...
int LA_OpenPGP_V20_ChangeUserPin (typeAPDU * tSC, int nSendLength,
                                  unsigned char *cSendData)
{
    int nRet;
...
int LA_OpenPGP_V20_ChangeAdminPin (typeAPDU * tSC, int nSendLength,
                                   unsigned char *cSendData)
{
    int nRet;
...
int LA_OpenPGP_V20_AES_Dec (typeAPDU * tSC, int nSendLength,
                            unsigned char *cSendData, int nReceiveLength,
                            unsigned char *cReceiveData)
{
    int nRet;
    ...
    {
        nRet =
            LA_OpenPGP_V20_AES_Dec_SUB (tSC, nSendLength, cSendData,
                                        nReceiveLength, cReceiveData);

        if ((0x90 != tSC->tState.cSW1) || (0x00 != tSC->tState.cSW2))   // Sc 
                                                                        // send 
                                                                        // no 
                                                                        // ok
        {
    ...
    if (32 == nSendLength)
    {
        ...
        nRet =
            LA_OpenPGP_V20_AES_Dec_SUB (tSC, 16, cSendData, 16, cReceiveData);
        ...
        nRet =
            LA_OpenPGP_V20_AES_Dec_SUB (tSC, 16, &cSendData[16], 16,
                                        &cReceiveData[16]);
...
#define AES_KEY_SIZE_IN_BYTE   32   // 16 = 128 Bit Achtung in
                                    // AES_StorageKeyEncryption aes size
                                    // anpassen
...
int LA_OpenPGP_V20_Test_ChangeUserPin (unsigned char *pcOldPin,
                                       unsigned char *pcNewPin)
{
    int nRet;
    int n;
    ...
    nRet =
        LA_OpenPGP_V20_ChangeUserPin (&tSC_OpenPGP_V20, n,
                                      (unsigned char *) acPinString);
    if (FALSE == nRet)
    {
        CI_LocalPrintf ("fail\n\r");
        return (FALSE);
...
int LA_OpenPGP_V20_Test_ChangeAdminPin (unsigned char *pcOldPin,
                                        unsigned char *pcNewPin)
{
    int nRet;
    int n;
    ...
    nRet =
        LA_OpenPGP_V20_ChangeAdminPin (&tSC_OpenPGP_V20, n,
                                       (unsigned char *) acPinString);
    if (FALSE == nRet)
    {
        CI_LocalPrintf ("fail\n\r");
...
int LA_OpenPGP_V20_Test_GetAID (unsigned char *cMainVersion,
                                unsigned char *cSecVersion)
{
    int nRet;
...
int LA_OpenPGP_V20_GetPasswordstatus (char *PasswordStatus)
{
    ...
    CI_LocalPrintf ("OK  P %d - A %d\n\r", tSC_OpenPGP_V20.cReceiveData[4],
                    tSC_OpenPGP_V20.cReceiveData[6]);
...
int LA_OpenPGP_V20_Test_SendAESMasterKey (int nLen,
                                          unsigned char *pcMasterKey)
{
    int nRet;
...
int LA_OpenPGP_V20_Test_LocalAESKey (void)
{
    ...
    AES_StorageKeyEncryption (AES_KEY_SIZE_IN_BYTE, acBuffer, acAES_MasterKey,
                              AES_PMODE_CIPHER);
    ...
    AES_StorageKeyEncryption (AES_KEY_SIZE_IN_BYTE, acBufferOut,
                              acAES_MasterKey, AES_PMODE_DECIPHER);
...
int LA_OpenPGP_V20_Test_ScAESKey (int nLen, unsigned char *pcKey)
{
    ...
    nRet =
        LA_OpenPGP_V20_AES_Dec (&tSC_OpenPGP_V20, nLen, pcKey, nLen,
                                acBufferOut);
    if (TRUE == nRet)
    {
...
int LA_OpenPGP_V20_Test_AES_Key (void)
{
    ...
    if (FALSE ==
        LA_OpenPGP_V20_Test_SendAdminPW ((unsigned char *) "12345678"))
    {
        return (FALSE);
    }

    if (FALSE ==
        LA_OpenPGP_V20_Test_SendAESMasterKey (AES_KEY_SIZE_IN_BYTE,
                                              acAES_MasterKey))
    {
        return (FALSE);
    }
    ...
    if (FALSE ==
        LA_OpenPGP_V20_Test_ScAESKey (AES_KEY_SIZE_IN_BYTE, acBuffer))
    {
        return (FALSE);
    }
...
int LA_OpenPGP_V20_ResetCard (void)
{
    ...
    {
        nRet =
            LA_OpenPGP_V20_Verify (&tSC_OpenPGP_V20, 1, 8,
                                   (unsigned char *) "@@@@@@@@");
        if (FALSE == nRet)
        {
        }
    ...
    {
        nRet =
            LA_OpenPGP_V20_Verify (&tSC_OpenPGP_V20, 3, 8,
                                   (unsigned char *) "@@@@@@@@");
        if (FALSE == nRet)
        {
        }
...
static u8 FlasgTimeIsSet_u8 = FALSE;
    ...
    Ret_u32 =
        LA_OpenPGP_V20_GetChallenge (&tSC_OpenPGP_V20, Size_u32, Data_pu8);
...
int LA_OpenPGP_V20_Test (void)
{
    ...
    // GETDATA 00 4F - Application identifier (AID), 16 (dec.) bytes (ISO
    // 7816-4)
    nRet = LA_OpenPGP_V20_GetData (&tSC_OpenPGP_V20, 0x00, 0x4F);
    if (FALSE == nRet)
    {
    ...
    // GETDATA 5F 52 - Historical bytes, up to 15 bytes (dec.), according to
    // ISO 7816-4. Card capabilities shall be included
    ...
    /*
     * nRet = LA_OpenPGP_V20_GetData (&tSC_OpenPGP_V20,0x00,0x5E); if (FALSE
     * == nRet) { return (FALSE); } 
     */
    ...
    if (FALSE == nRet)
    {
     ...
     /**/
        // VERIFY PIN 1
        nRet =
        LA_OpenPGP_V20_Verify (&tSC_OpenPGP_V20, 1, 6,
                               (unsigned char *) "123456");
    if (FALSE == nRet)
    {
        return (FALSE);
    }
    ...
    // VERIFY PIN 2
    nRet =
        LA_OpenPGP_V20_Verify (&tSC_OpenPGP_V20, 2, 6,
                               (unsigned char *) "123456");
    if (FALSE == nRet)
    {
        return (FALSE);
    ...
    LA_OpenPGP_V20_Decipher (&tSC_OpenPGP_V20, OPENPGP_V20_TEST_C_LENGTH,
                             cCryptogramTest, OPENPGP_V20_TEST_C_LENGTH,
                             cBuffer);
    if (FALSE == nRet)
    {
        return (FALSE);
...
u32 Ret_u32;
    ...
    Ret_u32 =
        LA_OpenPGP_V20_Verify (&tSC_OpenPGP_V20, PasswordFlag_u8,
                               strlen ((char *) String_pu8), String_pu8);
    if (FALSE == Ret_u32)
    {
...
u32 Ret_u32;
    ...
    {
        CI_LocalPrintf
            ("ERROR: GETDATA 00 4F - Application identifier (AID) \r\n");
        return (FALSE);
    }
...
void IBN_SC_Tests (unsigned char nParamsGet_u8, unsigned char CMD_u8,
                   unsigned int Param_u32, unsigned char *String_pu8)
{
    ...
    if (0 == nParamsGet_u8)
    {
        ...
        CI_LocalPrintf
            ("7  Kind    Show key:0=Signature,1=Confidentiality,2=Authentication\r\n");
        CI_LocalPrintf ("8          Signature erstellen\r\n");
        CI_LocalPrintf ("9          AES Key test\r\n");
        CI_LocalPrintf
            ("12         Change user pin old pin+new pin  -> 123456abcdef\r\n");
        CI_LocalPrintf
            ("13         Change admin pin old pin+new pin -> 12345678abcdefgh\r\n");
        CI_LocalPrintf ("14         SC startup test\r\n");
        CI_LocalPrintf ("15         Get password status\r\n");
        CI_LocalPrintf
            ("16 count   Generate [count] random numbers a 10 byte\r\n");
        CI_LocalPrintf
            ("17 count   Generate [count] *10 random numbers [output as 0,1 stream]\r\n");
        CI_LocalPrintf ("18 FiDi    FiDi Test\r\n");
        CI_LocalPrintf
            ("19 UserPW1 Reset error counter with PW1 (admin access)\r\n");
    ...
    switch (CMD_u8)
    {
        case 0:
            ...
            DelayMs (10);
            // GETDATA 00 4F - Application identifier (AID), 16 (dec.) bytes
            // (ISO 7816-4)
            Ret_u32 = LA_OpenPGP_V20_GetData (&tSC_OpenPGP_V20, 0x00, 0x4F);
            if (FALSE == Ret_u32)
            {
            ...
            DelayMs (10);
            // GETDATA 5F 52 - Historical bytes, up to 15 bytes (dec.),
            // according to ISO 7816-4. Card capabilities shall be included
            Ret_u32 = LA_OpenPGP_V20_GetData (&tSC_OpenPGP_V20, 0x5F, 0x52);
            if (FALSE == Ret_u32)
            {
                CI_LocalPrintf
                    ("ERROR: GETDATA 5F 52 - Historical bytes\r\n");
                return;
            }
            break;
        case 1:
            ...
            Ret_u32 = LA_OpenPGP_V20_Verify (&tSC_OpenPGP_V20, 1, strlen ((char *) LocalString_au8), LocalString_au8);  // 6, 
                                                                                                                        // (unsigned 
                                                                                                                        // char*)"123456");
            // Ret_u32 = LA_OpenPGP_V20_Verify
            // (&tSC_OpenPGP_V20,1,6,(unsigned char*)"123456");
            if (FALSE == Ret_u32)
            {
        ...
        case 2:
            ...
            // Ret_u32 = LA_OpenPGP_V20_Verify (&tSC_OpenPGP_V20,2,strlen
            // ((char*)LocalString_au8),LocalString_au8);
            // Ret_u32 = LA_OpenPGP_V20_Verify
            // (&tSC_OpenPGP_V20,2,6,(unsigned char*)"123456");
            if (FALSE == Ret_u32)
            {
                CI_LocalPrintf ("PW Error\r\n");
            }
        ...
        case 3:
            ...
            Ret_u32 =
                LA_OpenPGP_V20_Verify (&tSC_OpenPGP_V20, 3,
                                       strlen ((char *) LocalString_au8),
                                       LocalString_au8);
            // Ret_u32 = LA_OpenPGP_V20_Verify
            // (&tSC_OpenPGP_V20,3,8,(unsigned char*)"12345678");
            if (FALSE == Ret_u32)
            {
                CI_LocalPrintf ("PW Error\r\n");
            }
            break;
        case 4:
            // LA_OpenPGP_V20_Cipher
            // (&tSC_OpeEST_C_LENGPENPGP_V20_TEST_C_LENGTH,cCryptogramTest,OPENPGP_V20_TEST_C_LENGTH,cBuffer);
            break;
        case 5:
            LA_OpenPGP_V20_Decipher (&tSC_OpenPGP_V20,
                                     OPENPGP_V20_TEST_C_LENGTH,
                                     cCryptogramTest,
                                     OPENPGP_V20_TEST_C_LENGTH, cBuffer);
            break;
        ...
        case 7:    // Public key auslesen
            ...
            Ret_u32 =
                LA_OpenPGP_V20_GetPublicKey (&tSC_OpenPGP_V20, Param_u32,
                                             IBN_BUFFER_SIZE,
                                             &BufferPointer_pu8);
            if (TRUE == Ret_u32)
            {
                CI_LocalPrintf ("Key : Length %d\n\r",
                                tSC_OpenPGP_V20.nReceiveDataLen);
        ...
        case 8:    // Signature erstellen
            Ret_u32 =
                LA_OpenPGP_V20_ComputeSignature (&tSC_OpenPGP_V20, 10,
                                                 (unsigned char *)
                                                 "1234567890",
                                                 IBN_BUFFER_SIZE,
                                                 &BufferPointer_pu8);
        ...
        case 12:
            LA_OpenPGP_V20_Test_ChangeUserPin (String_pu8,
                                               (unsigned char *) "");
            break;
        case 13:
            LA_OpenPGP_V20_Test_ChangeAdminPin (String_pu8,
                                                (unsigned char *) "");
            break;
        case 14:
        ...
        case 17:   // Zufallszahl Dauertest
            CI_LocalPrintf
                ("Generate %d times 10 random chars  [Output as 0,1 stream]\n",
                 Param_u32);
    ...
    u32 i, i1;
        ...
        case 18:
            ...
            CI_LocalPrintf ("SC runtime %ld msec\n\r",
                            (u32) (Runtime_u64 /
                                   (u64) TIME_MEASURING_TICKS_IN_USEC /
                                   (u64) 1000L));
#endif
