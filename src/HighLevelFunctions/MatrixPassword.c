...
static u8 InitCalls_u8 = 200;   // First call generates random numbers
    ...
    if (FALSE ==
        GetRandomNumber_u32 (PWM_RAMDOMCHAR_BUFFERSIZE, PWM_Randomchars_au8))
    {
        CI_LocalPrintf
            ("GetRandomNumber_u32: Failed to get random numbers from smartcard\r\n");
    ...
    else
    {
        for (i = 0; i < PWM_RAMDOMCHAR_BUFFERSIZE; i++)
        {
            PWM_Randomchars_au8[i] = PWM_Randomchars_au8[i] % 10;   // Todo:
                                                                    // for
                                                                    // testing 
                                                                    // no
                                                                    // mymmetric 
    ...
    CCID_SmartcardOff_u8 ();    // To avoid smartcard error - after
                                // GetRandomNumber_u32
...
u8 Save_u8;
    ...
    for (PosY_u8 = 0; PosY_u8 < 10; PosY_u8++)
    {
        ...
        for (SwapX2ndPos_u8 = 0; SwapX2ndPos_u8 < 10; SwapX2ndPos_u8++)
        {
            ...
            if (PWM_PasswordMatrix_as8[SwapX2ndPos_u8][PosY_u8] !=
                SwapNumber_u8)
            {
                continue;
            }
            ...
            if (-1 == PWM_PasswordMatrix_as8[SwapX2ndPos_u8][BaseLineY_u8])
            {
                ...
                CI_LocalPrintf ("\r\nSwap X1 %d  Y1 %d - X2 %d Y2 %d\r\n",
                                SwapPosX_u8, BaseLineY_u8, SwapX2ndPos_u8,
                                PosY_u8);
#endif
                // Now we can swap
                // Swap the baseline
                PWM_PasswordMatrix_as8[SwapX2ndPos_u8][BaseLineY_u8] =
                    SwapNumber_u8;
                ...
                PWM_PasswordMatrix_as8[SwapX2ndPos_u8][PosY_u8] =
                    PWM_PasswordMatrix_as8[SwapPosX_u8][PosY_u8];
...
#ifdef DEBUG_MP_IO_LEVEL_2
                CI_LocalPrintf
                    ("MatrixError: Line x %d Number %d is missung\r\n",
                     PosY_u8, i);
#endif
                CheckOK_u8 = FALSE;
            }
...
#ifdef DEBUG_MP_IO_LEVEL_2
                CI_LocalPrintf
                    ("nMatrixError: Line y %d Number %d is missung\r\n",
                     PosX_u8, i);
#endif
                CheckOK_u8 = FALSE;
            }
...
#ifdef DEBUG_MP_IO_LEVEL_2
                CI_LocalPrintf
                    ("Matrix Error: Number_u8 %d - XPos_u8 %d - YPos_u8 %d\r\n",
