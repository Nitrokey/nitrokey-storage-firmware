 ...
 * Userpage layout
 * 
 * Byte 0 - 31 AES Storage key 32 - 51 Matrix columns for user password 52 -
 * 71 Matrix columns for admin password 72 - 101 Stick Configuration 102 -
 * 133 Base for AES key hidden volume (32 byte) 134 - 137 ID of sd card (4
 * byte) 138 - 141 Last stored real timestamp (4 byte) 142 - 145 ID of sc
 * card (4 byte) 146 - 177 XOR mask for sc tranfered keys (32 byte) 178 - 209 
 * Password safe key (32 byte)
 * 
 */
...
u8 WriteStickConfigurationToUserPage (void)
{
    ...
    flashc_memcpy (AVR32_FLASHC_USER_PAGE + 72, &StickConfiguration_st, 30,
                   TRUE);
    return (TRUE);
}
...
u32 ActiveSmartCardID_u32;
    ...
    memcpy (&StickConfiguration_st, (void *) (AVR32_FLASHC_USER_PAGE + 72),
    ...
    if (MAGIC_NUMBER_STICK20_CONFIG !=
        StickConfiguration_st.MagicNumber_StickConfig_u16)
    {
        return (FALSE);
    }
    return (TRUE);
...
cid_t *cid;
    ...
    memcpy (Status_st, &StickConfiguration_st, sizeof (typeStick20Configuration_st));   // Not 
                                                                                        // the 
                                                                                        // retry 
                                                                                        // counter 
                                                                                        // and 
    ...
    {
        Status_st->VolumeActiceFlag_u8 |=
            (1 << SD_UNCRYPTED_VOLUME_BIT_PLACE);
    }
...
u8 WriteNewSdCardFoundToFlash (u32 * SdId_u32)
{
    ...
    StickConfiguration_st.NewSDCardFound_u8 |= 0x01;    // Bit 0 = new card
                                                        // found
...
u8 SetSdCardFilledWithRandomsToFlash (void)
{
    ...
    StickConfiguration_st.SDFillWithRandomChars_u8 |= 0x01; // Set the "SD
                                                            // card filled
                                                            // with randoms"
                                                            // bit
...
u8 ClearNewSdCardFoundToFlash (void)
{
    ...
    StickConfiguration_st.NewSDCardFound_u8 &= 0xFE;    // Clear the "new SD
                                                        // card found" bit
...
u8 SetSdCardNotFilledWithRandomCharsToFlash (void)
{
    ...
    CI_LocalPrintf
        ("Set new SD card *** not *** filled with random chars\r\n");
...
u32 i1;
    ...
    for (i1 = 0; i1 < 5; i1++)
    {
        ...
        flashc_memcpy ((void *) AVR32_FLASHC_USER_PAGE, EraseStoreData_au8,
                       256, TRUE);
    }
    ...
    for (i1 = 0; i1 < 5; i1++)
    {
        ...
        flashc_memcpy ((void *) (PWS_FLASH_START_ADDRESS), EraseStoreData_au8,
                       256, TRUE);
        flashc_memcpy ((void *) (PWS_FLASH_START_ADDRESS + 256),
    ...
    for (i1 = 0; i1 < 5; i1++)
    {
        ...
        for (i = 0; i < 10; i++)
        {
            flashc_memcpy ((void *) (SLOTS_ADDRESS + i * 512),
                           EraseStoreData_au8, 256, TRUE);
            flashc_memcpy ((void *) (SLOTS_ADDRESS + i * 512 + 256),
                           EraseStoreData_au8, 256, TRUE);
        }
    }
    ...
    for (i1 = 0; i1 < 5; i1++)
    {
        ...
        for (i = 0; i < 2; i++)
        {
            flashc_memcpy ((void *) (HV_MAGIC_NUMBER_ADDRESS + i * 512),
                           EraseStoreData_au8, 256, TRUE);
            flashc_memcpy ((void *) (HV_MAGIC_NUMBER_ADDRESS + i * 512 + 256),
                           EraseStoreData_au8, 256, TRUE);
        }
    }
    for (i = 0; i < 10; i++)
