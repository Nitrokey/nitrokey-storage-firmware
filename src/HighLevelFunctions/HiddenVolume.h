...
// #define HV_MAGIC_NUMBER_SLOT_ENTRY 0x26f29c02 // Removed, because it is
// fix number, it could make it easier to attack the encrypted slot
...
typedef struct
{
    // u32 MagicNumber_u32; // Entry removed, because it is fix number, it
    // could make it easier to attack the encrypted slot
    u8 AesKey_au8[32];  // Move random chars in front, to harden struct
                        // against attacks
    u32 StartBlock_u32; // 
    u32 EndBlock_u32;   // 
    u32 Crc_u32;    // This had to be the last entry in struct
} HiddenVolumeKeySlot_tst;  // 44 byte = 4+4+32+4 - must multiple from 4 for
                            // CRC32
