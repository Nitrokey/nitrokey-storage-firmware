...
// Get last minute entry
  ...
  sprintf ((char*)Text_pu8,"%2d.%02d.%02d %2d:%02d:%02d - Min %4d Max %4d - %4d\r\n",tm_st.tm_mday,
                                                                    ...
                                                                    AD_Data_st.ValueMaxMinutes_u16[i] - AD_Data_st.ValueMinMinutes_u16[i]);
}
...
void AD_PrintSummary (void)
{
  ...
  for (i=AD_Data_st.PointerMinutesStart_u8;i!=AD_Data_st.PointerMinutesEnd_u8;i++)
  {
    ...
    localtime_r ((time_t *)&AD_Data_st.ValueMinutesTime_u32[i],&tm_st);
    CI_LocalPrintf ("%2d %2d: %02d.%02d.%02d %2d:%02d:%02d ",n,i,tm_st.tm_mday,tm_st.tm_mon+1,tm_st.tm_year-100,tm_st.tm_hour,tm_st.tm_min,tm_st.tm_sec);
  ...
  for (i=AD_Data_st.PointerHoursStart_u8;i!=AD_Data_st.PointerHoursEnd_u8;i++)
  {
    ...
    localtime_r ((time_t *)&AD_Data_st.ValueHoursTime_u16[i],&tm_st);
    CI_LocalPrintf ("%2d: %02d.%02d.%02d %2d:%02d:%02d ",n,tm_st.tm_mday,tm_st.tm_mon+1,tm_st.tm_year-100,tm_st.tm_hour,tm_st.tm_min,tm_st.tm_sec);
    CI_LocalPrintf ("min %4d  max %4d - %4d\r\n",AD_Data_st.ValueMinHours_u16[i],
                                                 AD_Data_st.ValueMaxHours_u16[i],
                                                 AD_Data_st.ValueMaxHours_u16[i] - AD_Data_st.ValueMinHours_u16[i]);
// Todo: find bug
...
/*
  switch (Filetype_u8)
  {
    case AD_FILEPATH_MINUTE :
      return (AD_FILEPATH_TXT_MINUTE);

    case AD_FILEPATH_HOUR   :
      return (AD_FILEPATH_TXT_HOUR);

    case AD_FILEPATH_DAY    :
      return (AD_FILEPATH_TXT_DAY);

    case AD_FILEPATH_EVENT  :
      return (AD_FILEPATH_TXT_EVENT);
...
u8 AD_FileIO_LogEvent_u8 (u32 time_u32)
{
  ...
  sprintf ((char*)Text_u8,"Event at %02d.%02d.%02d %2d:%02d:%02d\n",tm_st.tm_mday,tm_st.tm_mon+1,tm_st.tm_year-100,tm_st.tm_hour,tm_st.tm_min,tm_st.tm_sec);
...
//  sprintf ((char*)Text_u8,"%12ld - ",(unsigned long)LocalTime_u32);
  ...
  sprintf ((char*)Text_u8,"%12ld - %02d.%02d.%02d %2d:%02d:%02d ",(unsigned long)LocalTime_u32,tm_st.tm_mday,tm_st.tm_mon+1,tm_st.tm_year-100,tm_st.tm_hour,tm_st.tm_min,tm_st.tm_sec);
...
/*
  ...
  {
    CI_LocalPrintf ("AD_FileIO_SaveLocalTime_u8: fat_cache_flush FAIL - %d - %s\r\n",fs_g_status, IBN_FileSystemErrorText(fs_g_status));
  }
*/
  Ret_s32 = close (FileID_s32);
  if (-1 == Ret_s32)
...
//	FileID_s32 = open ("test.txt",O_CREAT|O_APPEND);
	FileID_s32 = open ((char*)Filename_pu8,O_CREAT|O_APPEND);
	if (0 > FileID_s32)
	{
		CI_LocalPrintf ("Error open %s - %d - %s\r\n",Filename_pu8,fs_g_status, IBN_FileSystemErrorText(fs_g_status));
...
//  CI_LocalPrintf ("%2d.%02d.%02d %2d:%02d:%02d %3d ms - ",tm_st.tm_mday,tm_st.tm_mon+1,tm_st.tm_year-100,tm_st.tm_hour,tm_st.tm_min,tm_st.tm_sec,now_ms);
