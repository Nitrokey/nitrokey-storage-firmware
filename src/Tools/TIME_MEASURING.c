...
u32 TIME_MEASURING_RuntimeSlice_in_ticks_u32 =
    TIME_MEASURING_RUNTIMESLICE_IN_TICKS;
u32 TIME_MEASURING_StoptimeSlice_in_ticks_u32 =
    TIME_MEASURING_STOPTIMESLICE_IN_TICKS;
...
u32 TIME_MEASURING_INT_RuntimeSlice_in_ticks_u32 =
    TIME_MEASURING_INT_RUNTIMESLICE_IN_TICKS;
u32 TIME_MEASURING_INT_StoptimeSlice_in_ticks_u32 =
    TIME_MEASURING_INT_STOPTIMESLICE_IN_TICKS;
...
static char String_u8[15];
    ...
    {
        sprintf (String_u8, "%6.2f ks",
                 (float) TimeInUsec_u64 / 1000000000.0);
        return (String_u8);
    }
...
void TIME_MEASURING_Start_st (TIME_MEASURING_tst * Time_st)
{
    ...
    {
        Time_st->LastStoptime_u32 =
            TIME_MEASURING_GetDiffTime (Time_st->LastStop_u64,
                                        Time_st->LastStart_u64);
...
u32 Value_u32;
    ...
    Time_st->LastRuntime_u64 =
        TIME_MEASURING_GetDiffTime (Time_st->LastStart_u64,
                                    Time_st->LastStop_u64);
    ...
    if (1000000000ull > (u64) Time_st->LastRuntime_u32) // log only runtime < 
                                                        // 1000/60 sec
    {
...
u32 TIME_MEASURING_Stop (u8 Timer_u8)
{
    ...
    return (TIME_MEASURING_Stop_st
            (&TIME_MEASURING_st[Timer_u8],
             TIME_MEASURING_RuntimeSlice_in_ticks_u32));
}
...
u32 i1;
    ...
    TIME_MEASURING_RuntimeSlice_in_ticks_u32 =
        NewSliceSizeInUsec_u32 * TIME_MEASURING_TICKS_IN_USEC;
...
*******************************************************************************/
/*
 * void TIME_MEASURING_SetStoptimeSliceSize (u32 NewSliceSizeInUsec_u32) {
 * u32 i; u32 i1;
 * 
 * TIME_MEASURING_StoptimeSlice_in_ticks_u32 = NewSliceSizeInUsec_u32 *
 * TIME_MEASURING_TICKS_IN_USEC;
 * 
 * for (i=0;i<TIME_MEASURING_MAX_ZEITSCHEIBEN;i++) { for
 * (i1=0;i1<TIME_MEASURING_MAX_TIME_SLICES;i1++) {
 * TIME_MEASURING_st[i].RuntimeSliceCount_u32[i1] = 0; } } }
 * 
 */
...
void TIME_MEASURING_PrintLine (u8 Entry_u8)
{
    ...
    CI_LocalPrintf (" %s : %9ld   ", TIME_MEASURING_TimerNames_u8[Entry_u8],
                    TIME_MEASURING_st[Entry_u8].Events_u32);

    if (0xFFFFFFFF != TIME_MEASURING_st[Entry_u8].LastRuntimeMin_u32)
    {
        CI_LocalPrintf ("%9ld ",
                        TIME_MEASURING_st[Entry_u8].LastRuntimeMin_u32 /
                        TIME_MEASURING_TICKS_IN_USEC);
    }
    else
    {
    ...
    CI_LocalPrintf ("%9ld  ",
                    TIME_MEASURING_st[Entry_u8].LastRuntimeMax_u32 /
                    TIME_MEASURING_TICKS_IN_USEC);

    if (0xFFFFFFFF != TIME_MEASURING_st[Entry_u8].LastStoptimeMin_u32)
    {
        CI_LocalPrintf ("%9ld ",
                        TIME_MEASURING_st[Entry_u8].LastStoptimeMin_u32 /
                        TIME_MEASURING_TICKS_IN_USEC);
    }
    else
    {
    ...
    CI_LocalPrintf ("%9ld ",
                    TIME_MEASURING_st[Entry_u8].LastStoptimeMax_u32 /
                    TIME_MEASURING_TICKS_IN_USEC);
    /*
     * CI_LocalPrintf ("\n\r%9ld
     * ",TIME_MEASURe_u32/[Entry_u8].SumRuntime_u32/TIME_MEASURING_TICKS_IN_USEC);
     * CI_LocalPrintf (" %9ld ",OS_Time); 
     */
    if (1 >= NoTimereset_u8)
    {
        CI_LocalPrintf ("%6.3f%%\n\r",
                        (float) (TIME_MEASURING_st[Entry_u8].SumRuntime_u64 /
                                 TIME_MEASURING_TICKS_IN_USEC) /
                        (float) xTaskGetTickCount () / 10.0);
    }
    else
    {
        CI_LocalPrintf ("\n\r");
...
void TIME_MEASURING_INT_show_interrupt_table (void)
{
    ...
    for (int_grp = 0; int_grp < AVR32_INTC_NUM_INT_GRPS; int_grp++)
    {
        ...
        for (int_req = 0; int_req < _int_handler_table[int_grp].num_irqs;
             int_req++)
        {
            if (&_unhandled_interrupt ==
                _int_handler_table[int_grp]._int_line_handler_table[int_req])
            {
                CI_LocalPrintf ("%3d %3d %4d %-15s : %9d UNHANDLED\n\r",
                                IntNamePointer, int_grp, int_req,
                                INT_NameTable[IntNamePointer],
...
void TIME_MEASURING_INT_show_interrupt_count (void)
{
    ...
    for (int_grp = 0; int_grp < AVR32_INTC_NUM_INT_GRPS; int_grp++)
    {
             ...
             int_req++)
        {
            ...
            {
                if ((__int_handler *) & _unhandled_interrupt ==
                    (__int_handler *) _int_handler_table[int_grp].
                    _int_line_handler_table[int_req])
                {
                    CI_LocalPrintf ("%3d %4d %-15s : %9d UNHANDLED\n\r",
                                    int_grp, int_req,
                                    INT_NameTable[IntNamePointer],
                                    TIME_MEASURING_INT_IntCount
                                    [IntNamePointer]);
                }
                else
                {
                    CI_LocalPrintf ("%3d %4d %-15s : %9d 0x%08x\n\r", int_grp,
                                    int_req, INT_NameTable[IntNamePointer],
                                    TIME_MEASURING_INT_IntCount
                                    [IntNamePointer],
                                    _int_handler_table[int_grp].
                                    _int_line_handler_table[int_req]);
    ...
    {
        CI_LocalPrintf ("             %-15s : %9d\n\r",
                        INT_NameTable[TIME_MEASURING_INT_UNKONWN],
                        TIME_MEASURING_INT_IntCount
                        [TIME_MEASURING_INT_UNKONWN]);
    }
}
...
void TIME_MEASURING_INT_SetTableOffsets (void)
{
    ...
    {
        /*
         * // For all interrupt request lines of each group, for (int_req =
         * 0; int_req < _int_handler_table[int_grp].num_irqs; int_req++) {
         * IntNamePointer++; } 
         */
...
void TIME_MEASURING_INT_Infos (unsigned char nParamsGet_u8,
                               unsigned char CMD_u8, unsigned int Param_u32,
                               unsigned char *String_pu8)
{
    if (0 == nParamsGet_u8)
    {
...
u32 TIME_MEASURING_INT_Stop (u8 Timer_u8)
{
    ...
    return (TIME_MEASURING_Stop_st
            (&INT_TIME_MEASURING_st[Timer_u8],
             TIME_MEASURING_INT_RuntimeSlice_in_ticks_u32));
#else
    return (0);
#endif
...
{
    CI_LocalPrintf ("%-17s %9ld   ",
                    TIME_MEASURING_IntTimerNames_u8[Entry_u8],
                    INT_TIME_MEASURING_st[Entry_u8].Events_u32);

    if (0xFFFFFFFF != INT_TIME_MEASURING_st[Entry_u8].LastRuntimeMin_u32)
    {
        CI_LocalPrintf ("%9ld ",
                        INT_TIME_MEASURING_st[Entry_u8].LastRuntimeMin_u32 /
                        TIME_MEASURING_TICKS_IN_USEC);
    }
    else
    {
    ...
    CI_LocalPrintf ("%9ld  ",
                    INT_TIME_MEASURING_st[Entry_u8].LastRuntimeMax_u32 /
                    TIME_MEASURING_TICKS_IN_USEC);

    if (0xFFFFFFFF != INT_TIME_MEASURING_st[Entry_u8].LastStoptimeMin_u32)
    {
        CI_LocalPrintf ("%9ld ",
                        INT_TIME_MEASURING_st[Entry_u8].LastStoptimeMin_u32 /
                        TIME_MEASURING_TICKS_IN_USEC);
    }
    else
    {
    ...
    CI_LocalPrintf ("%10ld ",
                    INT_TIME_MEASURING_st[Entry_u8].LastStoptimeMax_u32 /
                    TIME_MEASURING_TICKS_IN_USEC);

    if (1 >= NoTimereset_u8)
    {
        // CI_LocalPrintf
        // ("%5.3f%%\n\r",(floatURING_st[Entry_URING_st[Entry_u8].SumRuntime_u64/TIME_MEASURING_TICKS_IN_USEC)/(float)OS_Time/10);
    }
    else
    {
        CI_LocalPrintf ("\n\r");
...
void TIME_MEASURING_PrintInt (void)
{
    ...
    CI_LocalPrintf
        ("   Interrupt                      Runtime              Stoptime\n\r");
    CI_LocalPrintf
        (" Nr   Name           Anzahl  Min (usec) Max(usec) Min (usec)  Max(usec)\n\r");
...
u8 i;
    ...
    if (INT_TIME_MEASURING_MAX_ZEITSCHEIBEN <= Entry_u8)
    {
        CI_LocalPrintf ("ERROR: Entry %d not found. Max slot is %d\n\r",
                        Entry_u8, INT_TIME_MEASURING_MAX_ZEITSCHEIBEN - 1);
        return;
    }
    ...
    for (i = 0; i < TIME_MEASURING_MAX_TIME_SLICES - 1; i++)
    {
        CI_LocalPrintf (" %5ld - %5ld : %9ld\n\r",
                        TIME_MEASURING_INT_RuntimeSlice_in_ticks_u32 * i /
                        TIME_MEASURING_TICKS_IN_USEC,
                        TIME_MEASURING_INT_RuntimeSlice_in_ticks_u32 * (i +
                                                                        1) /
                        TIME_MEASURING_TICKS_IN_USEC,
                        INT_TIME_MEASURING_st[Entry_u8].
                        RuntimeSliceCount_u32[i]);
    }
    CI_LocalPrintf ("       > %5ld : %9ld\n\r",
                    TIME_MEASURING_INT_RuntimeSlice_in_ticks_u32 *
                    (TIME_MEASURING_MAX_TIME_SLICES -
                     1) / TIME_MEASURING_TICKS_IN_USEC,
                    INT_TIME_MEASURING_st[Entry_u8].
                    RuntimeSliceCount_u32[TIME_MEASURING_MAX_TIME_SLICES -
                                          1]);
}
...
u8 i;
    ...
    {
        CI_LocalPrintf ("ERROR: Entry %d not found. Max slot is %d\n\r",
                        Entry_u8, TIME_MEASURING_MAX_TIME_SLICES - 1);
        return;
    }

    CI_LocalPrintf ("Runtimeslot %s\n\r\n\r",
                    TIME_MEASURING_TimerNames_u8[Entry_u8]);

    CI_LocalPrintf ("Runtime (usec)          Count\n\r");
    for (i = 0; i < TIME_MEASURING_MAX_TIME_SLICES - 1; i++)
    {
        CI_LocalPrintf (" %5ld - %5ld : %9ld\n\r",
                        TIME_MEASURING_RuntimeSlice_in_ticks_u32 * i /
                        TIME_MEASURING_TICKS_IN_USEC,
                        TIME_MEASURING_RuntimeSlice_in_ticks_u32 * (i +
                                                                    1) /
                        TIME_MEASURING_TICKS_IN_USEC,
...
void TIME_MEASURING_Print (u8 Value_u8)
{
    ...
    CI_LocalPrintf
        ("                             Runtime              Stoptime\n\r");
    CI_LocalPrintf
        ("Counter        Anzahl  Min (usec) Max(usec) Min (usec) Max(usec)\n\r");
...
u32 time_ms (u32 * timer)
{
    ...
    {
        *timer =
            (u32) (TIME_MEASURING_GetTime () /
                   (u64) TIME_MEASURING_TICKS_IN_USEC / 1000LL);
        *timer %= 1000;
        return (*timer);
    }
    return (1);
