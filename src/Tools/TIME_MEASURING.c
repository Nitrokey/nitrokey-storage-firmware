...
/*
    CI_LocalPrintf ("\n\r%9ld ",TIME_MEASURING_st[Entry_u8].SumRuntime_u32/TIME_MEASURING_TICKS_IN_USEC);
    CI_LocalPrintf (" %9ld ",OS_Time);
*/
    if (1 >= NoTimereset_u8)
    {
        CI_LocalPrintf ("%6.3f%%\n\r",(float)(TIME_MEASURING_st[Entry_u8].SumRuntime_u64/TIME_MEASURING_TICKS_IN_USEC)/(float)xTaskGetTickCount()/10.0);
    }
    else
    {
        CI_LocalPrintf ("\n\r");
...
void TIME_MEASURING_INT_show_interrupt_table (void)
{
  ...
  for (int_grp = 0; int_grp < AVR32_INTC_NUM_INT_GRPS; int_grp++)
  {
    ...
    for (int_req = 0; int_req < _int_handler_table[int_grp].num_irqs; int_req++)
    {
      ...
      {
        CI_LocalPrintf ("%3d %3d %4d %-15s : %9d UNHANDLED\n\r",IntNamePointer,int_grp,int_req,INT_NameTable[IntNamePointer],TIME_MEASURING_INT_IntCount[IntNamePointer]);
      }
      else
      {
...
void TIME_MEASURING_INT_show_interrupt_count (void)
{
  ...
  for (int_grp = 0; int_grp < AVR32_INTC_NUM_INT_GRPS; int_grp++)
  {
    ...
    for (int_req = 0; int_req < _int_handler_table[int_grp].num_irqs; int_req++)
    {
      ...
      if (0 != TIME_MEASURING_INT_IntCount[IntNamePointer])
      {
        ...
        {
          CI_LocalPrintf ("%3d %4d %-15s : %9d UNHANDLED\n\r",int_grp,int_req,INT_NameTable[IntNamePointer],TIME_MEASURING_INT_IntCount[IntNamePointer]);
        }
        else
        {
          CI_LocalPrintf ("%3d %4d %-15s : %9d 0x%08x\n\r",int_grp,int_req,INT_NameTable[IntNamePointer],TIME_MEASURING_INT_IntCount[IntNamePointer],_int_handler_table[int_grp]._int_line_handler_table[int_req]);
        }
      }
      IntNamePointer++;
    }
  ...
  {
    CI_LocalPrintf ("             %-15s : %9d\n\r",INT_NameTable[TIME_MEASURING_INT_UNKONWN],TIME_MEASURING_INT_IntCount[TIME_MEASURING_INT_UNKONWN]);
  }
}
...
void TIME_MEASURING_PrintIntRuntimeSlots (u8 Entry_u8)
{
    ...
    CI_LocalPrintf ("       > %5ld : %9ld\n\r",TIME_MEASURING_INT_RuntimeSlice_in_ticks_u32*(TIME_MEASURING_MAX_TIME_SLICES-1)/TIME_MEASURING_TICKS_IN_USEC
...
void TIME_MEASURING_PrintRuntimeSlots (u8 Entry_u8)
{
    ...
    CI_LocalPrintf ("       > %5ld : %9ld\n\r",TIME_MEASURING_RuntimeSlice_in_ticks_u32*(TIME_MEASURING_MAX_TIME_SLICES-1)/TIME_MEASURING_TICKS_IN_USEC
