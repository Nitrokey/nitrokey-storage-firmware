...
int CI_TickLocalPrintf (char *szFormat, ...)
{
    ...
    nSize =
        vsnprintf ((char *) &szBuffer[8], MAX_TEXT_LENGTH - 1, szFormat,
                   args);
...
int CI_GetCommandLine (char *szCommandData, int nMaxTextLength)
{
    ...
    while (nCharPointer < nMaxTextLength - 1)
    {
        ...
        switch (n)
        {
            ...
            case 1:    // strg-a
                strcpy (szCommandData,
                        CI_GetStoredCommandLine (nOldCommandLinePointer));
                nCharPointer = strlen (szCommandData);
...
int CI_ExecCmd (char *szCommandLine)
{
    ...
    while (FALSE == End_u8)
    {
        ...
        {
            // Get number
            nValue[nParamsGet_u8] =
                (unsigned long) strtol ((char *) p, (char **) &p1, 10);
            if (p1 == p)    // End of line ?
            {
                End_u8 = TRUE;
                break;
    ...
    switch (nCmd)
    {
...
#else
            CI_LocalPrintf
                ("Test not enabled - activate ENABLE_IBN_FILE_ACCESS_TESTS\n\r");
#endif
        ...
        case CI_CMD_TOUCH:
            ...
            {
                CI_LocalPrintf
                    ("USAGE: touch filename (Yet, no numbers)\n\r");
                break;
            }
        ...
        case CI_CMD_MAKE_DIR:
            ...
            {
                CI_LocalPrintf
                    ("USAGE: mkdir filename (Yet, no numbers)\n\r");
                break;
            }
        ...
        case CI_CMD_TIME_TEST:
            ...
            IBN_TimeAccess (nParamsGet_u8, (u8) nValue[0], nValue[1],
                            String_pu8[0]);
#else
            CI_LocalPrintf
                ("Test not enabled - activate ENABLE_IBN_TIME_ACCESS_TESTS\n\r");
#endif
            break;

        case CI_CMD_USB:
            IBN_USB_Stats (nParamsGet_u8, (u8) nValue[0], nValue[1],
                           String_pu8[0]);
            break;
        case CI_CMD_SD:
            IBN_SD_Tests (nParamsGet_u8, (u8) nValue[0], nValue[1], nValue[2],
                          nValue[3]);
            break;
        case CI_CMD_SC:
            IBN_SC_Tests (nParamsGet_u8, (u8) nValue[0], nValue[1],
                          String_pu8[0]);
            break;
        case CI_CMD_HTML:
            IBN_HTML_Tests (nParamsGet_u8, (u8) nValue[0], nValue[1],
                            String_pu8[0]);
            break;
        case CI_CMD_HIGHLEVEL_TESTS:
            HighLevelTests (nParamsGet_u8, (u8) nValue[0], nValue[1],
                            String_pu8[0]);
            break;
        case CI_CMD_LED:
            IBN_LED_Tests (nParamsGet_u8, (u8) nValue[0], nValue[1],
                           String_pu8[0]);
            break;
        case CI_CMD_DFU:
#ifdef DEBUG_DUF_TESTS
            IBN_DFU_Tests (nParamsGet_u8, (u8) nValue[0], nValue[1],
                           String_pu8[0]);
#else
            CI_LocalPrintf
                ("Test not enabled - activate DEBUG_DUF_TESTS\n\r");
#endif
            break;
        case CI_CMD_INTTIME:
            TIME_MEASURING_INT_Infos (nParamsGet_u8, (u8) nValue[0],
                                      nValue[1], String_pu8[0]);
            break;
        case CI_CMD_FILE:
            IBN_FileIo_Tests (nParamsGet_u8, (u8) nValue[0], nValue[1],
                              nValue[2], nValue[3]);
            break;
        case CI_CMD_OTP:
            IBN_OTP_Tests (nParamsGet_u8, (u8) nValue[0], nValue[1],
                           String_pu8[0]);
            break;
        case CI_CMD_PWM:
#ifdef ENABLE_IBN_PWM_TESTS
            IBN_PWM_Tests (nParamsGet_u8, (u8) nValue[0], nValue[1],
                           String_pu8[0]);
#else
            CI_LocalPrintf
                ("Test not enabled - activate ENABLE_IBN_PWM_TESTS\n\r");
#endif
            break;
        case CI_CMD_HIDDEN_VOULME:
#ifdef ENABLE_IBN_HV_TESTS
            IBN_HV_Tests (nParamsGet_u8, (u8) nValue[0], nValue[1],
                          String_pu8[0]);
#else
            CI_LocalPrintf
                ("Test not enabled - activate ENABLE_IBN_HV_TESTS\n\r");
#endif
            break;
        case CI_CMD_PASSWORD_SAFE:
            IBN_PWS_Tests (nParamsGet_u8, (u8) nValue[0], nValue[1],
                           String_pu8[0]);
            break;
...
void IDF_task (void *pvParameters)
{
    ...
    while (TRUE)
    {
        ...
        {
            if (sd_LastAccessedBlockReadMin_u32 <
                sd_MaxAccessedBlockReadMin_u32)
            {
                CI_LocalPrintf
                    ("Highest sd block accessed read : MIN %ld = %ld MB\n\r",
                     sd_MaxAccessedBlockReadMin_u32,
                     sd_MaxAccessedBlockReadMin_u32 / 2000);

                sd_LastAccessedBlockReadMin_u32 =
                    sd_MaxAccessedBlockReadMin_u32;
            }
            if (sd_LastAccessedBlockWriteMin_u32 <
                sd_MaxAccessedBlockWriteMin_u32)
            {
                CI_LocalPrintf
                    ("Highest sd block accessed write : MIN %ld = %ld MB\n\r",
                     sd_MaxAccessedBlockWriteMin_u32,
                     sd_MaxAccessedBlockWriteMin_u32 / 2000);

                sd_LastAccessedBlockWriteMin_u32 =
                    sd_MaxAccessedBlockWriteMin_u32;
            }

            if (sd_LastAccessedBlockReadMax_u32 >
                sd_MaxAccessedBlockReadMax_u32)
            {
                CI_LocalPrintf
                    ("Highest sd block accessed read : MAX %ld = %ld MB\n\r",
                     sd_MaxAccessedBlockReadMax_u32,
                     sd_MaxAccessedBlockReadMax_u32 / 2000);

                sd_LastAccessedBlockReadMax_u32 =
                    sd_MaxAccessedBlockReadMax_u32;
            }
            if (sd_LastAccessedBlockWriteMax_u32 <
                sd_MaxAccessedBlockWriteMax_u32)
            {
                CI_LocalPrintf
                    ("Highest sd block accessed write : MAX %ld = %ld MB\n\r",
                     sd_MaxAccessedBlockWriteMax_u32,
                     sd_MaxAccessedBlockWriteMax_u32 / 2000);

                sd_LastAccessedBlockWriteMax_u32 =
                    sd_MaxAccessedBlockWriteMax_u32;
            }
        }
...
void IDF_task_init (void)
{
    ...
    xTaskCreate (IDF_task,
                 ...
                 NULL, configTSK_IDF_TEST_PRIORITY, NULL);
}
