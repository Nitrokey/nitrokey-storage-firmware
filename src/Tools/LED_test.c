...
static volatile u8 LED_StartUpSdCardOnline = FALSE; // Offline = red LED
                                                    // flashing on
...
void LED_RedGreenToggle (void)
{
    ...
    {
        /*
         * if (LED_OFF == LED_RedToggleFlag_u8) { gpio_clr_gpio_pin
         * (TOOL_LED_RED_PIN); } if (LED_OFF == LED_GreenToggleFlag_u8) {
         * gpio_clr_gpio_pin (TOOL_LED_GREEN_PIN); } 
         */
...
{
    /*
     * if (LED_OFF == LED_GreenToggleFlag_u8) { gpio_clr_gpio_pin
     * (TOOL_LED_GREEN_PIN); } if (LED_OFF == LED_RedToggleFlag_u8) {
     * gpio_clr_gpio_pin (TOOL_LED_RED_PIN); } 
     */
    LED_RedGreenToggleFlag_u8 = LED_OFF;
}
...
{
    /*
     * gpio_set_gpio_pin (TOOL_LED_RED_PIN); gpio_set_gpio_pin
     * (TOOL_LED_GREEN_PIN); 
     */
...
void LED_Manager10ms_v (void)
{
    ...
    if (TRUE == LED_StartUpActiv)   // Startup usage
    {
        ...
        if ((TRUE == LED_StartUpSdCardOnline)
            && (TRUE == LED_StartUpScCardOnline))
        {
    ...
    {   // Normal LED usage
        ...
        if (LED_ON == LED_RedGreenToggleFlag_u8)    // Red-green has higher
                                                    // prio
        {
            StateRedLed_u8 = LED_ON;
...
void IBN_LED_Tests (unsigned char nParamsGet_u8, unsigned char CMD_u8,
