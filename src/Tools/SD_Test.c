...
u16 i;
    ...
    if (FALSE == GetSdEncryptedCardEnableState ())  // Flag for enabling the
                                                    // sd card
    {
...
u32 i;
    ...
    CI_TickLocalPrintf
        ("SD_WriteMultipleBlocks - Addr %x Block %d Count %d \r\n", Addr_u8,
         Block_u32, Count_u32);
    ...
    for (i = 0; i < Count_u32; i++)
    {
        // Addr_u8[0] = (u8)i;
        if (FALSE ==
            sd_mmc_mci_dma_write_open (SD_SLOT, Block_u32, Addr_u8, 1))
        {
...
void SD_WriteMultipleBlocksWithRandoms (u8 * Addr_u8, u32 Block_u32,
                                        u32 Count_u32, u8 CryptionFlag_u8)
{
u32 i;
...
u64 Runtime_u64;
    ...
    CI_TickLocalPrintf
        ("SD_WriteMultipleBlocksWithRandoms - Addr %x Block %d Count %d \r\n",
    ...
    for (i = 0; i < Count_u32; i++)
    {
        ...
        if (TRUE == CryptionFlag_u8)
        {
            ...
            dma_ram_2_mci_crypted (Addr_u8, SD_MMC_SECTOR_SIZE,
                                   Block_u32 + i);
        }
        else
        {
        ...
        if (i % 1000 == 0)
        {
            ...
            if (TRUE == CryptionFlag_u8)
            {
                AES_SetNewStorageKey ((u8 *) & Addr_u8[i % (256 - 16)]);    // Get 
                                                                            // key 
                                                                            // from 
                                                                            // input 
                                                                            // data
        ...
        {
            UpdateStick20Command (OUTPUT_CMD_STICK20_STATUS_BUSY_PROGRESSBAR,
                                  (u8) (i / (Count_u32 / 100)));
        }
        ...
        if (i % 10000 == 0)
        {
            if (0 != i)
            {
                ...
                Runtime_u64 =
                    TIME_MEASURING_GetLastRuntimeU64 (TIME_MEASURING_TIME_15);
                CI_LocalPrintf
                    ("%8d Blocks : SD runtime %ld sec = %6.0f kb/sec\n\r", i,
                     (u32) (Runtime_u64 / (u64) TIME_MEASURING_TICKS_IN_USEC /
                            (u64) 1000000L),
                     (float) ((u64) i * (u64) SD_MMC_SECTOR_SIZE) /
                     (float) (Runtime_u64 /
    ...
    CI_LocalPrintf ("SD runtime %ld sec = %6.0f kb/sec\n\r",
                    (u32) (Runtime_u64 / (u64) TIME_MEASURING_TICKS_IN_USEC /
                           (u64) 1000000L),
                    (float) ((u64) Count_u32 * (u64) SD_MMC_SECTOR_SIZE) /
                    (float) (Runtime_u64 /
                             (u64) TIME_MEASURING_TICKS_IN_USEC) * 1000.0);
#endif
}
...
u16 i;
    ...
    for (i = 0; i < SD_BLOCK_SIZE / SC_RANDOM_SIZE; i++)
    {
        ...
        if (FALSE ==
            GetRandomNumber_u32 (SC_RANDOM_SIZE,
                                 &sector_buf_0[i * SC_RANDOM_SIZE]))
        {
...
u32 i;
    ...
    if (CCID_SLOT_STATUS_PRESENT_ACTIVE != CCID_GetSlotStatus_u8 ())
    {
        ...
        {
            CI_TickLocalPrintf
                ("SD_GetRandomBlock: Can't start smartcard\r\n");
            return (FALSE);
        }
    }
    ...
    for (i = 0; i < SD_BLOCK_SIZE / SC_RANDOM_SIZE; i++)
    {
        ...
        if (FALSE ==
            GetRandomNumber_u32 (SC_RANDOM_SIZE,
                                 &RandomData_u8[i * SC_RANDOM_SIZE]))
        {
    ...
    /*
     * CI_TickLocalPrintf ("\r\nRandom Block = \r\n"); for
     * (i=0;i<SD_BLOCK_SIZE/SC_RANDOM_SIZE;i++) { CI_LocalPrintf ("0x%03x :
     * ",i*SC_RANDOM_SIZE); HexPrint
     * (SC_RANDODOSIZE,&RandomData_u8[i*SC_RANDOM_SIZE]); CI_LocalPrintf
     * ("\r\n"); } 
     */
    return (TRUE);
}
...
u16 i;
    ...
    // Write Data
    SD_WriteMultipleBlocksWithRandoms (RandomBuffer_au8, Block_u32, Count_u32,
                                       CryptionFlag_u8);
...
*******************************************************************************/
#define SD_SPEED_TEST_START_BLOCK     ((1024L*1024L*1024L)/SD_MMC_SECTOR_SIZE)  // at 
                                                                                // 1 
                                                                                // GB
#define SD_SPEED_TEST_BLOCK_COUNT     ((10L*1024L*1024L)/SD_MMC_SECTOR_SIZE)    // 10 
                                                                                // MB
...
u8 Ret_u8;
    ...
    /*
     * #ifdef TIME_MEASURING_ENABLE Runtime_u64 = TIME_MEASURING_GetTime ();
     * #endif Ret_u8 = SD_WriteMultipleBlocks
     * (RandomBuffer_au8,SD_SPEED_TEST_START_BLOCK,SD_SPEED_TEST_BLOCK_COUNT); 
     * // First access could be slower than normal #ifdef
     * TIME_MEASURING_ENABLE Runtime_u64 = TIME_MEASURING_GetTime () -
     * Runtime_u64; SpeedInKBPerSec_u16 =
     * (u16)((float)((u64)SD_SPEED_TEST_BLOCK_COUNT*(u64)SD_MMC_SECTOR_SIZE)
     * /(float)(Runtime_u64/(u64)TIME_MEASURING_TICKS_IN_USEC)*1000.0 );
     * CI_LocalPrintf ("SD runtime %ld msec = %6.0f kb/sec = %d
     * kb/sec\n\r",(u32)(Runtime_u64/(u64)TIME_MEASURING_TICKS_IN_USEC/(u64)1000L),(float)((u64)SD_SPEED_TEST_BLOCK_COUNT*(u64)SD_MMC_SECTOR_SIZE) 
     * /(float)(Runtime_u64/(u64)TIME_MEASURING_TICKS_IN_USEC)*1000.0,SpeedInKBPerSec_u16 
     * ); #endif
     * 
     * vTaskDelay (100); // Wait until no access while (TRUE ==
     * Check_Sd_mmc_mci_access_signal_on ()) { } 
     */
...
#endif
    Ret_u8 =
        SD_WriteMultipleBlocks (RandomBuffer_au8, SD_SPEED_TEST_START_BLOCK,
                                SD_SPEED_TEST_BLOCK_COUNT);
...
#endif
        Ret_u8 =
            SD_WriteMultipleBlocks (RandomBuffer_au8,
                                    SD_SPEED_TEST_START_BLOCK,
                                    SD_SPEED_TEST_BLOCK_COUNT);
    }
    ...
    SpeedInKBPerSec_u16 =
        (u16) ((float)
               ((u64) SD_SPEED_TEST_BLOCK_COUNT * (u64) SD_MMC_SECTOR_SIZE) /
               (float) (Runtime_u64 / (u64) TIME_MEASURING_TICKS_IN_USEC) *
               1000.0);
    CI_LocalPrintf ("SD runtime %ld msec = %6.0f kb/sec = %d kb/sec\n\r",
                    (u32) (Runtime_u64 / (u64) TIME_MEASURING_TICKS_IN_USEC /
                           (u64) 1000L),
                    (float) ((u64) SD_SPEED_TEST_BLOCK_COUNT *
                             (u64) SD_MMC_SECTOR_SIZE) /
                    (float) (Runtime_u64 /
                             (u64) TIME_MEASURING_TICKS_IN_USEC) * 1000.0,
                    SpeedInKBPerSec_u16);
#endif
...
void IBN_SD_Tests (u8 nParamsGet_u8, u8 CMD_u8, u32 Param_u32,
                   u32 Param_1_u32, u32 Param_2_u32)
{
...
const cid_t *cid;
    ...
    if (0 == nParamsGet_u8)
    {
        ...
        CI_LocalPrintf
            ("9    Erase encrypted volume with random numbers via AES\r\n");
        ...
        CI_LocalPrintf
            ("17 X Set new SD card found 0 = found, 1 = clear found\r\n");
        CI_LocalPrintf ("\r\n");
        return;
    }
    ...
    switch (CMD_u8)
    {
        case 0:
             ...
             * CI_LocalPrintf ("unre %x\r\n",mci->SR.unre ); CI_LocalPrintf
             * ("ovre %x\r\n",mci->SR.ovre ); CI_LocalPrintf ("xfrdone
             * %x\r\n",mci->SR.xfrdone ); CI_LocalPrintf ("fifoempty
             * %x\r\n",mci->SR.fifoempty ); CI_LocalPrintf ("dmadone
             * %x\r\n",mci->SR.dmadone ); CI_LocalPrintf ("blkovre
             * %x\r\n",mci->SR.blkovre ); CI_LocalPrintf ("cstoe
             * %x\r\n",mci->SR.cstoe ); CI_LocalPrintf ("dtoe
             * %x\r\n",mci->SR.dtoe ); CI_LocalPrintf ("dcrce
             * %x\r\n",mci->SR.dcrce ); CI_LocalPrintf ("rtoe
             * %x\r\n",mci->SR.rtoe ); CI_LocalPrintf ("rende
             * %x\r\n",mci->SR.rende ); CI_LocalPrintf ("rcrce
             * %x\r\n",mci->SR.rcrce ); CI_LocalPrintf ("rdire
             * %x\r\n",mci->SR.rdire ); CI_LocalPrintf ("rinde
             * %x\r\n",mci->SR.rinde ); CI_LocalPrintf ("txbufe
             * %x\r\n",mci->SR.txbufe ); CI_LocalPrintf ("rxbuff
             * %x\r\n",mci->SR.rxbuff ); CI_LocalPrintf ("csrcv
             * %x\r\n",mci->SR.csrcv ); CI_LocalPrintf ("sdiowait
             * %x\r\n",mci->SR.sdiowait ); CI_LocalPrintf ("sdioirqd
             * %x\r\n",mci->SR.sdioirqd ); CI_LocalPrintf ("sdioirqc
             * %x\r\n",mci->SR.sdioirqc ); CI_LocalPrintf ("sdioirqb
             * %x\r\n",mci->SR.sdioirqb ); CI_LocalPrintf ("sdioirqa
             * %x\r\n",mci->SR.sdioirqa ); CI_LocalPrintf ("endtx
             * %x\r\n",mci->SR.endtx ); CI_LocalPrintf ("endrx
             * %x\r\n",mci->SR.endrx ); CI_LocalPrintf ("notbusy
             * %x\r\n",mci->SR.notbusy ); CI_LocalPrintf ("dtip
             * %x\r\n",mci->SR.dtip ); CI_LocalPrintf ("blke
             * %x\r\n",mci->SR.blke ); CI_LocalPrintf ("txrdy
             * %x\r\n",mci->SR.txrdy ); CI_LocalPrintf ("rxrdy
             * %x\r\n",mci->SR.rxrdy ); CI_LocalPrintf ("cmdrdy
             * %x\r\n",mci->SR.cmdrdy ); 
             */
            break;
        ...
        case 7:
            ...
            break;
        case 8:
            sd_mmc_mci_read_capacity (SD_SLOT,
                                      (unsigned long int *) &Blockcount_u32);
        ...
        case 14:
            ...
            CI_LocalPrintf ("Product name        %c%c%c%c%c\r\n", cid->pnmh,
                            (cid->pnml >> 24) & 0xff,
                            (cid->pnml >> 16) & 0xff, (cid->pnml >> 8) & 0xff,
                            (cid->pnml >> 0) & 0xff);
...
max = (u64) sd_MaxAccessedBlockReadMax_u32 *(u64) 100 / (u64) Blockcount_u32;
                ...
                CI_LocalPrintf ("Read  MIN %3d %% - block %u\r\n", (u8) min,
                                sd_MaxAccessedBlockReadMin_u32);
                CI_LocalPrintf ("Read  MAX %3d %% - block %u\r\n", (u8) max,
                                sd_MaxAccessedBlockReadMax_u32);
...
max = (u64) sd_MaxAccessedBlockWriteMax_u32 *(u64) 100 / (u64) Blockcount_u32;
                ...
                CI_LocalPrintf ("Write MIN %3d %% - block %u\r\n", (u8) min,
                                sd_MaxAccessedBlockWriteMin_u32);
                CI_LocalPrintf ("Write MAX %3d %% - block %u\r\n", (u8) max,
                                sd_MaxAccessedBlockWriteMax_u32);
            }
            else
            {
u32 Blockcount_u32;
