...
void IBN_GetTaskList (void)
{
    ...
    do
    {
        ...
        {
            IBN_GetTaskListEntrys (&(pxReadyTasksLists[Queue_u32]),
                                   IBN_TASK_STATE_RUNNING);
        }
    ...
    {
        IBN_GetTaskListEntrys (pxOverflowDelayedTaskList,
                               IBN_TASK_STATE_DELAYED_OVER);
    }
...
void IBN_StoreTaskRuntimeData (void)
{
    ...
    {
        IBN_Tasklist_st[i]->ullRunTimeCounterLastMinute =
            IBN_Tasklist_st[i]->ullRunTimeCounterMinute;
...
u16 usStackRemaining;
    ...
    usStackRemaining =
        usTaskCheckFreeStackSpace ((unsigned char *) IBN_Tasklist_st[i]->
                                   pxStack) * sizeof (portSTACK_TYPE);
    ullStatsAsPercentage = (u64) 0;

    if ((u64) 0 != ullTotalRunTime)
    {
ullStatsAsPercentage =
            (u64) 10000 *(u64) (IBN_Tasklist_st[i]->ullRunTimeCounter);
        ullStatsAsPercentage /= ullTotalRunTime;

    }

    CI_LocalPrintf ((char *) "%-21.21s ", IBN_Tasklist_st[i]->pcTaskName);
    CI_LocalPrintf ((char *) "%s   ",
                    IBN_TaskStates[IBN_TasklistState_u8[i]]);
    CI_LocalPrintf ((char *) "%u  ",
                    (unsigned int) IBN_Tasklist_st[i]->uxPriority);
    CI_LocalPrintf ((char *) "%5u ", usStackRemaining);
    // CI_LocalPrintf( ( char * ) "%10u ",
    // (u32)(IBN_Tasklist_st[i]->ullRunTimeCounter / 60ULL));
    CI_LocalPrintf ("%s ",
                    TIME_MEASURING_ShortText (IBN_Tasklist_st[i]->
                                              ullRunTimeCounter / 60ULL));

    CI_LocalPrintf ((char *) "%6.2f %% ",
                    (float) ullStatsAsPercentage / 100.0);
    CI_LocalPrintf ((char *) "%6.2f %% ",
                    (float) IBN_Tasklist_st[i]->ullRunTimeCounterLastMinute /
                    (10000.0 * 60.0 * 60.0) * (float) (configTICK_RATE_HZ) /
                    1000.0);
...
void IBN_ListTaskWithinSingleList (xList * pxList, char *pcStatus)
{
    ...
    ullStatsAsPercentage = (u64) 10000 *(u64) (pxNextTCB->ullRunTimeCounter);
        ...
        CI_LocalPrintf ("%10u ",
                        (u32) (pxNextTCB->ullRunTimeCounter / 60ULL));
        CI_LocalPrintf ("%s ",
                        TIME_MEASURING_ShortText (pxNextTCB->
                                                  ullRunTimeCounter / 60ULL));
        ...
        // pxNextTCB->pcTaskName, pcStatus, ( unsigned int )
        // pxNextTCB->uxPriority,
        // usStackRemaiimeCountextTCB->ullRunTimeCounter/60,(float)ullStatsAsPercentage/100.0);
    } while (pxNextTCB != pxFirstTCB);
}
...
void IBN_ShowTaskStatusAll (void)
{
    ...
    CI_LocalPrintf ("Tick      : %d - %d sec\r\n", xTickCount,
                    xTickCount / configTICK_RATE_HZ);
    CI_LocalPrintf
        ("                                   Stack      Runtime      Last Minute\r\n");
    CI_LocalPrintf
        ("Taskname              State   Prio  free    usec       %%        %%      Starts\r\n");
...
void IBN_ShowHeadinfo (void)
{
    ...
    CI_LocalPrintf ("HEAP free      %d\r\n",
                    (&__heap_end__ - &__heap_start__) - (info.usmblks +
                                                         info.uordblks));
#endif
}
...
char *IBN_FileSystemErrorText (u8 FS_Error_u8)
{
    switch (FS_Error_u8)
    {
        ...
        case FS_ERR_NO_FORMAT:
            return
                ("FS_ERR_NO_FORMAT        The selected drive isn't formated");
        case FS_ERR_NO_PART:
            return
                ("FS_ERR_NO_PART          The selected partition doesn't existed");
        case FS_ERR_NO_SUPPORT_PART:
            return
                ("FS_ERR_NO_SUPPORT_PART  The selected partition isn't supported");
        case FS_ERR_TOO_FILE_OPEN:
            return
                ("FS_ERR_TOO_FILE_OPEN    The navigation have already opened a file");
        ...
        case FS_ERR_ENTRY_BADTYPE:
            return
                ("FS_ERR_ENTRY_BADTYPE    File entry type don't corresponding");
        case FS_ERR_NO_DIR:
            return
                ("FS_ERR_NO_DIR           The selected file isn't a directory");
        ...
        case FS_NO_LAST_LFN_ENTRY:
            return
                ("FS_NO_LAST_LFN_ENTRY    The file entry isn't the last long file entry");
        case FS_ERR_ID_FILE:
            return ("FS_ERR_ID_FILE          The file identifier is bad");
        case FS_ERR_NO_FILE:
            return
                ("FS_ERR_NO_FILE          The selected file entry isn't a file");
        case FS_LUN_WP:
        ...
        case FS_ERR_NAME_INCORRECT:
            return
                ("FS_ERR_NAME_INCORRECT   The name don't corresponding at the filter name");
        ...
        case FS_ERR_IS_ROOT:
            return
                ("FS_ERR_IS_ROOT          There aren't parent because the current directory is a root directory");
        case FS_ERR_OUT_LIST:
            return
                ("FS_ERR_OUT_LIST         The position is outside the cluster list");
        case FS_ERR_NO_FREE_SPACE:
            return ("FS_ERR_NO_FREE_SPACE    No free cluster found in FAT");
        case FS_ERR_INCORRECT_NAME:
            return
                ("FS_ERR_INCORRECT_NAME   Incorrect name, this one cannot contain any of the following characters");
        case FS_ERR_DIR_NOT_EMPTY:
            return
                ("FS_ERR_DIR_NOT_EMPTY    This function erases only file and empty directory");
        ...
        case FS_ERR_BAD_SIZE_FAT:
            return
                ("FS_ERR_BAD_SIZE_FAT     The disk size is not supported by selected FAT format");
        case FS_ERR_COMMAND:
        ...
        case FS_ERR_COPY_DIR:
            return
                ("FS_ERR_COPY_DIR         Directory copy is not supported");
        case FS_ERR_COPY_RUNNING:
            return
                ("FS_ERR_COPY_RUNNING     A copy action is always running");
        case FS_ERR_COPY_IMPOSSIBLE:
            return ("FS_ERR_COPY_IMPOSSIBLE  The copy is impossible");
        case FS_ERR_BAD_NAV:
            return
                ("FS_ERR_BAD_NAV          The navigator identifier doesn't existed");
        case FS_ERR_FILE_OPEN:
            return ("FS_ERR_FILE_OPEN        The file is already opened");
        case FS_ERR_FILE_OPEN_WR:
            return
                ("FS_ERR_FILE_OPEN_WR     The file is already opened in write mode");
        case FS_ERR_FILE_EXIST:
            return ("FS_ERR_FILE_EXIST       The file is already existed");
        case FS_ERR_NAME_TOO_LARGE:
            return
                ("FS_ERR_NAME_TOO_LARGE   The file name is too large (>260 characters)");
        case FS_ERR_DEVICE_TOO_SMALL:
            return
                ("FS_ERR_DEVICE_TOO_SMALL The disk size is too small for format routine");
        case FS_ERR_PL_NOT_OPEN:
            return ("FS_ERR_PL_NOT_OPEN      The play list isn't opened");
        case FS_ERR_PL_ALREADY_OPEN:
            return
                ("FS_ERR_PL_ALREADY_OPEN  The play list is already opened");
        case FS_ERR_PL_LST_END:
            return
                ("FS_ERR_PL_LST_END       You are at the end of play list");
        case FS_ERR_PL_LST_BEG:
            return
                ("FS_ERR_PL_LST_BEG       You are at the beginning of play list");
        case FS_ERR_PL_OUT_LST:
            return
                ("FS_ERR_PL_OUT_LST       You are outside of the play list");
        case FS_ERR_PL_READ_ONLY:
            return
                ("FS_ERR_PL_READ_ONLY     Impossible to modify the play list");
        default:
    ...
    switch (CMD_u8)
    {
        ...
        case 2:
            ...
            CI_LocalPrintf ("Set date to %2d.%02d.%04d\r\n", tm_st.tm_mday,
                            tm_st.tm_mon + 1, 1900 + tm_st.tm_year);
            now = mktime (&tm_st);
            set_time (now);
            break;
        case 3:
