...
// Get block kind
	...
	switch (nBlockKind)
	{
		case ISO7816_T1_BLOCK_KIND_I_BLOCK :
			...
			{
			  if ((ISO7816_T1_SendBuffer[ISO7816_T1_PCB_ADDR] & ISO7816_T1_PCB_CHAINING_BIT_I_BLOCK) != 0)    // Todo check: When no chaining bis is set
			  {
			    return (ISO7816_T1_RECEIVE_BLOCK_ERROR);
			  }
			}
...
// check send counter
/* 	Check Full PCB
	if ((ISO7816_T1_SendBuffer[ISO7816_T1_PCB_ADDR] & ISO7816_T1_PCB_SEND_NR_BIT_I_BLOCK) !=
	    (ISO7816_T1_GetLastSendNr () * ISO7816_T1_PCB_SEND_NR_BIT_I_BLOCK))
	{
		return (FALSE);
	}
*/
...
/*
	if (ISO7816_USART_TIMEOUT == nRet)
	{
		CI_LocalPrintf ("%7d : ISO7816_T1_DirectXfr - Timeout\n",xTaskGetTickCount());
	}
*/
