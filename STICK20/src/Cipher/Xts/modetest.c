...
enum des_type { EKY  = 0, TKY, LBA, PTX, CTX,               /* sequence values      */
                VEC = 16, END, REM, SLN, NUL, RPT, ERR };   /* non-sequence values  */

char *des_name[] =      { "EKY ", "TKY ", "LBA ", "PTX ", "CTX ", "SLN ", "VEC ", "END", "REM ", "RPT " };
enum des_type des_m[] = {  EKY  ,  TKY  ,  LBA  ,  PTX  ,  CTX  ,  SLN  ,  VEC  ,  END ,  REM  ,  RPT   };
...
{   char            tvi_name[64], tvo_name[64], line[80], *lp;
    ...
    int             tky_len, eky_len, sln_len, lba_len, ptx_len, ctx_len,
                    ptx_rpt, rpt_cnt, vec_no, *cnt, *rpt = &rpt_cnt, err, i;
        ...
        for( ; ; )
        {
            ...
            case RPT:   *rpt = dec_in(lp);
        				if(gen_flag)
		        		    fprintf(outf, "\nRPT %04i", *rpt);
                        continue;
            case EKY:   p = eky, cnt = &eky_len, rpt = &rpt_cnt; break;
            case TKY:   p = tky, cnt = &tky_len, rpt = &rpt_cnt; break;
            case SLN:   p = sln, cnt = &sln_len, rpt = &rpt_cnt; break;
            case LBA:   p = lba, cnt = &lba_len, rpt = &rpt_cnt; break;
