...
SET /a count=1 
FOR /f "skip=1 delims=:" %%a IN ('CertUtil -hashfile %TEMP%/firmware_orig_filled.bin SHA512') DO (
...
SET /a count=1 
for /f "skip=1 delims=:" %%a IN ('CertUtil -hashfile %TEMP%/firmware_exported_filled.bin SHA512') DO (
...
ECHO Original File SHA512: %orig_hash%
ECHO Exported File SHA512: %exp_hash%
